...
For replaceall  $\replaceall_{p}(sub, rep)$ where $p$ is a fixed regular expression, we notice that, by the result in Section~\ref{appendix:replace2pt}, the transducer $\Transducer_p$ modelling this function is of exponential size in that of $p$, i.e., $|\Transducer_p|\in \bigO(2^{|p|})$. By Theorem~\ref{thm-generic-dec}, the decision procedure requires
$\bigO(|\Transducer_p|^m \rcpsi(S)^m)=\bigO(2^{|p|\cdot m}\rcpsi(S)^m)$ space (where $m=\rcdep(S)$), which is still exponential.  
%
%
