...
The notion of runs of 2NFA on an input string is exactly the same as that of
...
is
sequence of pairs $(q_0,i_0),\ldots, (q_m,i_m) \in \controls \times (\{0\} \cup [n+1])$ 
...
For convenience, we will also refer to an NFA without initial and final states, that is, a pair $(Q, \delta)$, as a \emph{transition graph}.
...
2NFA and NFA recognise precisely the same class of languages, i.e., 
...
\begin{proposition}[\cite{Kozen-automata}]\label{prop-2nfa-nfa}
	Every 2NFA $\Aut$ is equivalent to an NFA of size $2^{O(|\Aut| \log |\Aut|)}$. 
...
\paragraph{Operations of NFAs.} For an NFA $\Aut=(Q, q_0, F, \delta)$, $q \in Q$ and $P \subseteq Q$, we use $\Aut(q, P)$ to denote the NFA $(Q, q, P, \delta)$, that is, the NFA obtained from $\Aut$ by changing the initial state and the set of final states to $q$ and $P$ respectively. We use $q \xrightarrow[\Aut]{w} q'$ to denote the fact that a string $w$ is accepted by $\Aut(q, \{q'\})$. 
