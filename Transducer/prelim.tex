...
respectively. For $k \in \Nat$, let $[k] = \{0,\cdots, k\}$. For a vector
...
\defn{alphabet}). A sequence over $\ialphabet$ is called a \defn{word}
(also, a \defn{string}). A \defn{language} is set of words over $\ialphabet$.
...
the initial state, $\finals\subseteq \controls$ is a set of final states, and 
    $\transrel$ is the
transition relation  of type $\transrel\subseteq \controls \times 
    \bar{\ialphabet}\times \{\Left, \Stay, \Right\}\times \controls$ with
    $\bar{\ialphabet} := \ialphabet \cup \{\EndLeft,\EndRight\}$.
...
A word $w$ is \defn{accepted} by $\Aut$ if there is an accepting run of
$\Aut$ on $w$. The set of words accepted by $\Aut$ is denoted by $\Lang(\Aut)$,
...
The proof is by now standard and can be found in textbooks on automata theory
