...
\paragraph{Computational complexity.} In this paper, we will study %decision procedures and 
(theoretical) computational complexity of the path feasibility problem for programs with strings. 
%============ add if space
...
We will deal with the following complexity
classes (assuming a Turing machine model; see \cite{HU79} for more details): $\expspace$ (problems solvable
in exponential space), $n$-$\expspace$ (problems solvable in $n$-fold exponential space), {\sc nonelementary} (problems that cannot be solved in $n$-fold exponential
