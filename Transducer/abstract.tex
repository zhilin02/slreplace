...
%logics for reasoning about symbolic executions of programs with strings. 
    ...
    as an expressive formalism for modelling many useful string functions.
    These include string functions that are crucial for such an application as 
    analysis of cross-site scripting (XSS) vulnerabilities in HTML5 
    applications including sanitisation functions (e.g.~htmlescape) and 
    implicit browser transductions (e.g.~innerhtml).  
    Thanks to their good algorithmic properties, it was recently shown that 
    finite-state transductions can be incorporated into decidable
    ...
    finite-state transductions lack the expressive power for modelling many
    useful string functions; three notable examples being the replace-all 
    function (replacing occurrence of replacing all occurrences of a ``pattern''
    string constant/variable/regular expression by a ``replacement'' string 
    constant/variable), the string reverse
    function, and string concatenation. 
    In this paper we introduce \emph{parameterised transducers}, an expressive
    framework for modelling string functions that strictly 
    generalise the standard finite-state transducer model and capture the three
    aforementioned string functions, among others. We provide a decidable
    string logic with parameterised transducers as the primitive operations,
    and provide a precise computational complexity analysis. This gives us
    a highly expressive theory over strings that subsumes existing string 
