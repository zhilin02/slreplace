...
%  $q'_{s,0}= p$, $q'_{s,j} \xrightarrow[\Aut]{w'_j} q'_{s, j+1}$ for each $j \in [m]$,  and $q'_{s, m+1} = p'$, 
 %
 %there is a state sequence $q'_0, \cdots, q'_{m+1} \in Q'$ such that $q'_{m+1} \in F'$, and for each $j \in \{0\} \cup [m]$, if $\gamma_j \in \Sigma$, then $(q'_j, \gamma_j, q'_{j+1}) \in \delta'$, otherwise, let $\gamma_j =x_i$, then $(q'_j, q'_{j+1}) \in S_{x_i}$.
\end{itemize}
...
for each $(p, p') \in S$, $(q_0, p) \xrightarrow[\Aut'']{w} (q, p')$. Thus, $w$ is accepted by $\Aut''$. Since  $\cB_{\Transducer, S_{x_1}, \cdots, S_{x_k}, q}$ is the 1NFA equivalent to $\Aut''$, we conclude that $w$ is accepted by $\cB_{\Transducer, S_{x_1}, \cdots, S_{x_k}, q}$.
 ...
 %Moreover, for each $i \in [k]$, $w_i $ is accepted by $\Aut[S_{x_i}]$.
...
\noindent {\bf Proposition~\ref{prop-2nst-nst}}. \emph{Each 2NST can be turned into an equivalent NST}.
