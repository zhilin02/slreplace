...
Thus, we use two numbers to measure the size of $\Transducer$: The number of transitions of $\Transducer$, denoted by $\numtrans(\Transducer)$, and the maximum size of transitions of $\Transducer$, denoted by $\sizetrans(\Transducer)$.
...
The two size parameters of 2SPT can be defined similarly to 2ST.
...
\end{definition}
The semantics of 2SA  is defined similarly to 2ST, with the output data values removed. Moreover, the two size parameters can also be defined similarly to 2ST.


\begin{proposition}\label{prop-2nsa}
The following results hold.
\begin{itemize}
\item Every 2SA $\Aut$ can be transformed effectively into an equivalent SA $\Aut'$, in time $O(\numtrans(\Aut') \sizetrans(\Aut'))$, where  
$\numtrans(\Aut') = 2^{O\left(\numtrans(\Aut) \log \numtrans(\Aut)\right)}$ and $\sizetrans(\Aut') = O( \numtrans(\Aut) \sizetrans(\Aut))$.
%
%in time $$2^{O\left(\numtrans(\Aut) \log \numtrans(\Aut)\right)}O( \numtrans(\Aut) \sizetrans(\Aut)),$$

%
\item SAs can be determinized and are closed under all Boolean operations. \zhilin{complexity}
%
\item The nonemptiness of SA is decidable. \zhilin{complexity}
\end{itemize}
\end{proposition}
