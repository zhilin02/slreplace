...
In this section, we consider an extension of parametric transducers, called \emph{symbolic parametric transducers}, where the input and output letters are specified symbolically,
%infinite data domains \tl{maybe not emphasise infinite domain}, , 
more precisely, the input letters are implicitly described by unary predicates  in some logical theory and the output letters are given by terms, where the parameters may occur. 
%
...
In sequel, we first define symbolic automata and transducers, then introduce symbolic parametric transducers and show how to extend the results in preceding sections to the symbolic setting.
...
Concepts of Boolean algebra and background theories are used in the definition of symbolic automata and transducers.
%and label theories. 
Intuitively, a Boolean algebra is a set of unary first-order logic formulae closed under Boolean connectives, and a  background theory is a Boolean algebra  closed under substitutions. 
 %Label theories extend background theories further by adding inequalities of terms into the set of formulae.
 
 
 \begin{definition}[Boolean algebra and background theories]
A Boolean algebra  $\Upsilon$ is a tuple $(\signature, \structure, \Psi)$ satisfying the following constraints:
\begin{itemize}
...
A formula $\psi \in \Psi$ is \emph{satisfiable}, denoted by $\issat(\psi)$, if $\|\psi\| \neq \emptyset$. A Boolean algebra or background theory $\Upsilon = (\signature, \structure, \Psi)$ is \emph{decidable} if  checking  $\issat(\psi)$ for a  given $\psi \in\Psi$ is decidable.
\end{definition}
...
\begin{definition}[Symbolic automata]
    A (nondeterministic two-way)  symbolic automata (\SSA) is a tuple $\Aut = (\Upsilon, \EndLeft, \EndRight, \controls, q_0, \finals, \transrel)$, where  
\begin{itemize}
...
%
\item     $(q, \EndLeft, dir, q')$ with $dir \in \{\Right, \Stay\}$, or $(q, \EndRight, dir', q')$ with $dir' \in \{\Left,\Stay\}$. 
\end{itemize}
\end{itemize}
A 2SA is an SA if there are no transitions with the direction $\{\Left\}$. 
...
%
%For a background theory $\Upsilon=(\signature, \structure, \Psi)$ with $\structure = (\data, \interpretation)$, 
...
\begin{itemize}
  ...
  \end{itemize}
\end{itemize}
The run is said to be \defn{accepting} if $i_m = n+1$ and $q_m \in \finals$. When a run is accepting, $d'_0, \ldots, d'_m$ is the \emph{output} of the run.
...
%the fact that each transition of $\Transducer$ may contain a guard and a term, which are .
%Thus, we use two numbers for this purpose: The number of transitions of $\Transducer$, denoted by $\numtrans(\Transducer)$, and the maximum size of transitions of $\Transducer$, denoted by $\sizetrans(\Transducer)$.
%
%Thus, we define the size of $\Transducer$, denoted by $|\Transducer|$, as the sum of the sizes of (the binary encodings of) transitions of $\Transducer$.
For a refined complexity analysis, we use the following two measures: %numbers to capture the size of $\Transducer$:  Define 
...
\tl{but they are sill not defined?}
...
\tl{warning: there was a typo: Y was X}
\begin{itemize}
...
\paragraph{Symbolic extension of the generic procedure.} To solve the satisfiability problem of $\straightlinesym[\transet]$, the generic decision procedure in Section~\ref{sec:algo} can be smoothly extended, by replacing recognisable relations with symbolically recognisable relations defined below.
...
\paragraph{Symbolic extension of pre-image computation.}
