...
\end{definition}
We would like to remark that in literature, an \SA{} is defined on a Boolean algebra, which is a background theory, with the constraint that $\Psi$ is closed under substitutions dropped. We choose to define \SSA{}s and \SA{}s on background theories, instead of Boolean algebras, in Definition~\ref{def-2sa}, since symbolic parametric transducers, the focus of this section, will be defined on background theories.
...
$\numtrans(\Aut)$, the number of transitions of $\Aut$, and $\sizetrans(\Aut)$, the maximum size of symbolic transitions of $\Aut$.
...
%
\item the nonemptiness of $\Aut$ can be decided in nondeterministic $\mathcal{O}( (\log \numtrans(\Aut)) + f(\sizetrans(\Aut)))$ space.
\end{itemize}
\end{proposition}
...
Similarly to \SSA{}s, we define two measures of a \SST{} $\Transducer$: %numbers to capture the size of $\Transducer$:  Define 
...
$\vec{t}$ is a sequence of $\signature$-terms whose variables are from $\{x, y_1,\ldots, y_m\}$,
%
...
$w_1,\ldots, w_m$ gives rise to a \SST{} by replacing each occurrence of $y_i$ in transitions with $w_i$. 
...
%
The size of \SSPT{}s can be defined in a similar way as \SST{}s. 
