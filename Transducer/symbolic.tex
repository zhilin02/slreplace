...
\end{definition}
We remark that, in literature, an \SA{} is defined on a Boolean algebra, which is a background theory without requiring $\Psi$ to be closed under substitutions dropped. We choose to define \SSA{}s and \SA{}s on background theories (rather than Boolean algebras) in Definition~\ref{def-2sa} to align to the symbolic parametric transducers defined later.
