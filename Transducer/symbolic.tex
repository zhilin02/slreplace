...
The path feasibility of data string manipulating programs, where the data string functions are given by \SSPT{}s, can be decided in nondeterministic $\tower(\rcdep(S), \bigO(|S|^r)) + \beta(\tower(\rcdep(S), \bigO(|S|^r)) )$  space for some constant $r > 0$. On the other hand,  the path feasibility for \SPT{}s can be decided in nondeterministic $|S|^{\bigO(\rcdep(S))}+ \beta(|S|^{\bigO(\rcdep(S))})$ space.
\end{theorem}
