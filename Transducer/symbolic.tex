...
%
The size of a symbolic transition $(q_j, \psi(x), dir, q_{j+1}, \vec{t}(x))$ in a \SSA{} with $\vec{t}=t_1 \ldots t_r$ is defined as $|\psi| + |t_1| + \ldots + |t_r|$.
The control size $|\Transducer|_c$ and the data size $|\Transducer|_d$ of \SST{}s are defined similarly to those of \SSA{}s. 
...
\paragraph{Symbolic extension of the generic decision procedure.} To solve the path feasibility problem of string manipulating programs in SSA form, the generic decision procedure in Section~\ref{sec:algo} can be smoothly extended, by replacing recognisable relations and the \prerec{} assumption with symbolically recognisable relations  and \prerec{}' assumption presented below.
...
\end{quote} 
%
Here $|f|_c$ and $|f|_d$ are the control and data sizes of a representation of $f$ respectively; the concrete definitions depend on the form of $f$, which will be given when the generic decision procedure is instantiated later.

Let $S$ be a data string manipulating program in SSA form.  Then $\rcdep(S)$ and $\rcdim(S)$ are defined similarly to the string setting. Moreover, we use $\rcphi_c(S)$ (resp. $\rcphi_d(S)$) to denote the maximum of $|f|_c$ (resp. $|f|_d$) for string functions $f$ occurring in the assignments of $S$, moreover, we use $\rcpsi_c(S)$ (resp. $\rcpsi_d(S)$) to denote the maximum of $|\Aut|_c$ (resp. $|\Aut|_d$) for \SA{}s $\Aut$ occurring in $S$. Finally, we define $\rcsreg(S)$ to be the maximum number of atomic \SA{} constraints for one variable in $S$.
...
\begin{theorem}
path feasibility of $\straightlinesym[\twspt]$ can be decided in $M + f(M)$ space, where $M = 2^{\left. 2^{\ldots ^{2^{O(p(|\varphi| |\psi|))}} }\right\}\rcdep(\varphi)}$ for some polynomial $p$. On the other hand,  path feasibility of $\straightlinesym[\owspt]$ can be decided in $N+ f(N)$ space, where $N = |\psi| 2^{O(|\varphi|\log |\varphi|)}$.
\end{theorem}
