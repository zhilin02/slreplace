...
Let $\Transducer$ be a 2NPT and $\Aut$ be a succinctly encoded NFA $\Aut$. Then $\Pre_\Transducer(\Aut)$ is a recognisable relation. Moreover, a representation of $\Pre_\Transducer(\Aut)$, whose atom size is bounded by $2^{O(|\Transducer||\Aut|\log (|\Transducer||\Aut|))}$, can be computed effectively.
\end{lemma}

\begin{proof}
Assume a 2NPT $\Transducer=(X, Q, q_0, F, \delta)$ with $X = \{x_1,\cdots, x_k\}$ and a succinctly encoded NFA $\Aut = ((Q', \delta'), S)$ with $S \subseteq Q' \times Q'$. %We first introduce some notations.
