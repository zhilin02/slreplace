...
The existence of a solution to a tiling problem of width $n$ can be reduced to checking whether a 2T  of size 
$\ap{\bigoh}{n}$ 
can output a specified symbol $\top$.
Equivalently, we could use a 2NFA.
...
To encode tiling problems of $n$-fold exponential width we chain $(n+1)$ 2T.
We begin with two 2Ts and a tiling problem of exponential width.
...
Given such an encoding, the first 2T checks the solution in a similar manner to the width $n$ problem.
To cope with the large corridor width, it will output ``queries'' to the second 2T.
These queries check whether one binary number is equal to the next, or is the increment.
Now, the first 2T can, for example, non-deterministically guess that one tile is in the same column as another and use queries to verify this guess.
...
To check a given tiling, the first 2T outputs queries to the second.
To be able to check exponentially long binary numbers, the second 2T outputs queries to the third 2T.
This process can be iterated so that using $(n+1)$ 2Ts, we can search for a solution of a tiling problem over an $n$-fold exponentially wide corridor.
