...
Furthermore, from Proposition~\ref{prop-replace-pt}, we know that  $\Transducer_{\replaceall_p}$ is of size $2^{\bigO(|p|^c)}$ for some constant $c > 0$.  Combining this fact, Theorem~\ref{thm-generic-dec}, and Lemma~\ref{lem-1pt}  together, we have:
