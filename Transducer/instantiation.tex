...
\end{theorem}
%
To see Theorem~\ref{thm-rb-2pt}, we invoke Theorem~\ref{thm-generic-dec} again. One can observe that the space required by the instantiation of the generic procedure is $\bigO(\rcphi(S)^{k+k^2+...+k^m}\cdot  \rcpsi(S)^{2^m})$ (where $m=\rcdep(S)$), which is doubly exponential in the size of $S$. 
