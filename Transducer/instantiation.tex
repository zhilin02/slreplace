...
From Section \ref{sec-2way}  and Section \ref{sec-one-way}, we see that one-way transducers enjoy much lower complexity, but their expressiveness is limited. (For instance, the reverse function is inexpressible.) On the other hand, two-way transducers suffer from very higher complexity. We observe that, in practice, many string functions, when encoded by transducers, need only a small number of reversals for the reading head, rather than the full power of two-way transducers. We shall consider reversal bounded transducers, which remain the complexity advantages of one-way transducers, but which can capture a larger class of string functions than one-way transducers. 
...
Evidently, the class of $0$-reversal bounded \PPT{}s is precisely the class of \PT{}s. Note that the number of reversals in accepting runs of \PPT{}s must be even, since they must stop on the right end of input strings.  As demonstrated in Example~\ref{exmp-ft} (in Section~\ref{sec:framework}), string reverse function can be captured by a $2$-reversal bounded \PPT.

%To handle $k$-reversal bounded transducers, 
We observe that any string manipulating program (in SSA forms) with string functions modelled by general $k$-reversal bounded transducers can be translated to the one with only one-way transducers and a special transducer $\Transducer_{\sf reverse}$ for the reverse function.  

\begin{proposition}
	Given string manipulating program $S$ where each string function is given by a $k$-reversal bounded transducer, one can construct $S'$ where each string function is given by a one-way transducer or  $\Transducer_{\sf reverse}$ and $ \rcdep(S')\in \mathcal{O}(k\cdot \rcdep(S))$. 
\end{proposition}

%Our goal is to show the following result.
%
To instantiate the generic procedure as before, one only needs to verify the  \prerec{} assumption for $\Transducer_{\sf reverse}$. Indeed, although $\Transducer_{\sf reverse}$ is a two-way transducer, its pre-image does \emph{not} bring exponential blowup as general two-way transducers would have (cf. Lemma~\ref{lem-2pt}), but rather it is akin to one-way transducers. 
...
Combing these results together, we have 
