...
A \emph{symbolic transducer}(\SST) is a \SSPT{} where $Y = \emptyset$ (then $Y$ is omitted).  A \emph{symbolic automaton} (\SSA) is a symbolic transducer where each transition has the  empty output (then the outputs are omitted).
\end{definition}
...
Then the regularity condition in Section~\ref{sec:algo} is replaced by the \emph{symbolic} regularity condition that for each function $f: (\data^*)^\arity \to \data^*$, the pre-image of a language definable by an \SA{} under $f$ is a symbolic recognisable relation and a representation of which can be computed.
...
\begin{theorem}
Given a symbolic execution $S$ of programs with data strings wherein string functions are given by \SSPT{}s (resp. \SPT{}s), the path feasibility of $S$ is decidable.
\end{theorem}
