...
By the premise of the theorem, $\Pre_{R_f}(\Aut_j)$ is a recognisable relation and a representation of which, say $(\Aut^{(\iota)}_{j, 0}, \Aut^{(\iota)}_{j, 1}, \ldots, \Aut^{(\iota)}_{j, k})_{\iota \in [m_j]}$, \tl{zhilin: why the index starts from 0? it is a $k$-ary relation, isn't it?} can be computed effectively.
