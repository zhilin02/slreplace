...
Therefore, for each $x \in \vars(S)$, the product of these conjunctive \FA{}s  in $\cE(x)$ is of size $(\rcdim(S)+1)^{\rcdep(S)} \ell^{\langle \rcdep(S) \rangle}(\rcphi(S), \rcpsi(S))$. Since the nonemptiness of a conjunctive \FA{} can be solved in polynomial space, we conclude that the aforementioned nondeterministic algorithm takes 
