...
$f(|\Transducer|, |\Aut|)$. Then the satisfiability of $\straightline[\transet]$ formula $\varphi \wedge \psi$ can be decided in \emph{nondeterministic} 
%\tl{why not remove  nondeterministic?} 
$O((K+2)^D f^{(D)}(|\varphi|, |\psi|))$ space, where $K$ is the maximum number of parameters of 
%$\Transducer \in \transet$, 
...
emptiness checking can be done in nondeterministic logarithmic space in the size of the product automaton, we conclude that the satisfiability can be checked by a nondeterministic Turing machine with %is in nondeterministic 
