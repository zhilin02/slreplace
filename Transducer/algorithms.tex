...
Furthermore, to obtain tight space complexity bounds (in some cases), we do the following transformation:
%
%Each representation can be turned into one satisfying the requirement as follows: 
Suppose $\Aut^{(i)}_j=(\controls^{(i)}_j, q^{(i)}_{0,j}, \transrel^{(i)}_j, \finals^{(i)}_j)$ for $i \in [n]$ and $j \in [k]$. Since $\Lang(\Aut^{(i)}_j) = \bigcup \limits_{q \in \finals^{(i)}_j} \Lang(\Aut^{(i)}_j(q^{(i)}_{0,j}, \{q\}))$, we have 
\begin{align*}
...
\end{align*}
%
Notice that each NFA $\Aut^{(i)}_j(q^{(i)}_{0, j}, \{q^{(i)}_j\})$ can be written as a pair $((\controls^{(i)}_j,  \transrel^{(i)}_j), \{(q^{(i)}_{0,j}, q^{(i)}_j)\})$.
...
From Lemma~\ref{lem-2pt} and Theorem~\ref{thm-generic-dec}, we deduce that the satisfiability of an $\straightline[\twpt]$ formula $\varphi \wedge \psi$ can be decided in nondeterministic $f^{\langle \rcdep(\varphi)\rangle}(|\varphi|, |\psi|)$ space, where $f(j, k) = 2^{jk^2 \log (j k)}$. Therefore, the satisfiability of an $\straightline[\twpt]$ formula $\varphi \wedge \psi$ can be decided in nondeterministic $\tower(\rcdep(\varphi), O(p(|\varphi| + |\psi|)))$ space for some polynomial $p$, thus in (deterministic) $\tower(\rcdep(\varphi), O(p(|\varphi| + |\psi|)))$ space, according to Savitch's theorem \cite{Savitch70}. Here $\tower(j, k)$ is a tower of $2$s of height $j$ with $k$ on top, formally, $\tower(1, k)= 2^k$ and $\tower(j+1, k) = 2^{\tower(j, k)}$. We conclude that the satisfiability of $\straightline[\twpt]$ formula $\varphi \wedge \psi$ can be decided in $\rcdep(\varphi)$-exponential space.
