...
From Lemma~\ref{lem-2pt}, Theorem~\ref{thm-generic-dec}, we deduce that the satisfiability of an $\straightline[\twpt]$ formula $\varphi \wedge \psi$ can be decided in nondeterministic $\rcdep(\varphi)$-tower exponential space, thus  in (deterministic) $\rcdep(\varphi)$-tower exponential space, according to Savitch's theorem \cite{Savitch70}.
%
\begin{theorem}
Given an $\straightline[\twpt]$ formula $\varphi \wedge \psi$, its satisfiability can be decided in $\rcdep(\varphi)$-tower exponential space. Moreover,  the satisfiability of $\straightline[\twpt]$ is non-elementary.
\end{theorem}

The non-elementary lower bound is proved as follows. \zhilin{@Matt, could you help put your lower bound proof here ?}
