...
\end{array}
$$
where $x_{e} = a$ and $ x_{([a^{*?}])} = \varepsilon$, namely, $e $ is matched to ``$a$''  and  $([a^{*?}])$ is matched to $\varepsilon$. Therefore, the semantics of $e$ defined by $\cT^\prime_e$  is \emph{inconsistent} with semantics of /($a$*?)*/ in node.js.
%}
%
	\begin{figure}[tb]
%		\vspace{-2mm}
