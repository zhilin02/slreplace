...
The main idea of the construction is to split the set of final states, $F$, into two disjoint subsets $F_1$ and $F_2$, with the intention that $F_1$ and $F_2$ are responsible for accepting the empty string resp. non-empty strings. To ensure this, making copies of {\PSST} might be needed during the construction, as is done in concatenation of two {\PSST}s (Definition \ref{def-psstconcat}), which leads to the worst case exponential blowup.

However, the separation of $F$ into two sets is necessary for correct modeling of {\regexp} semantics. This is justified by the observation that in real world languages like JavaScript, the behaviour of certain {\regexp} operator, like greedy Kleene star $e^*$, is influenced by whether the subexpression $e$ matches an empty string. For example, the expression $(a^{*?})^*$ either matches $\varepsilon$ holistically and in this case the subexpression $a^{*?}$ is not entered and matched at all, or it matches the concatenation of a number of \emph{non-empty} substrings which are accepted by $a^{*?}$. Note that this is an instance of problematic regular expression in \cite{BDM14, BM17}'s setting. By giving a more precise semantics to those operators and keep track of if their subexpression is matched to $\varepsilon$, we can successfully handle this class of expression.

Note here for technical convenience, we assume that $F$ in a {\PSST} is a set of final states, instead of an output function. Therefore, the {\PSST}s constructed below are of the form $(Q, \Sigma, X, \delta, \tau, E, q_0, (F_1, F_2))$. 
%
...
Note that the construction is technical and below we only select to present some representative cases. The other cases are
%
...
\begin{remark}
	Extract and replace use regexes with non-classical semantics and references in replace. Pre-image recognizability was not known; previous results consider classical regular expressions without references (a non-example is Example 1.1). PSSTs elegantly capture the new semantics and prove pre-image recognizability. Handling lookahead/backreferences is non-trivial, partly due to the undecidability of the resulting SL fragment (e.g. with backreferences). We can investigate alternating PSSTs or product constructions for lookahead, and extend PSSTs to inspect variable contents for backreferences.
\end{remark}
\color{black}
