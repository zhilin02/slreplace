...
The main idea of the construction is to split the set of final states, $F$, into two disjoint subsets $F_1$ and $F_2$, with the intention that $F_1$ and $F_2$ are responsible for accepting the empty string resp. non-empty strings. The necessity of this splitting will be illustrated in Example~\ref{exmp-psst-partition}.
...
For technical convenience, we assume that $F$ in a {\PSST} is a set of final states, instead of an output function. Therefore, the {\PSST}s constructed below are of the form $(Q, \Sigma, X, \delta, \tau, E, q_0, (F_1, F_2))$. 
%
...
At last, we use the following example to illustrate the necessity of splitting the set of final states into two disjoint subsets.

\begin{example}\label{exmp-psst-partition}
Consider {\pcre} $e = [([a^{*?}])^{*}]$. If we execute ``$aaa$''.match(/($a$*?)*/) in node.js, then the result is the array $[$``aaa'', ``a''$]$, where ($a$*?)* is matched to ``$aaa$'' and ($a$*?) is matched to $a$. If we do not split the set of final states into two disjoint subsets, then we obtain a PSST $\cT^\prime_e$ as illustrated in Fig.~\ref{fig-psst-partition}. The accepting run of $\cT^\prime_e$ on $w  = aaa$ is 
%
%{\small
$$
\begin{array}{l}
\smallskip
q_{[([a^{*?}])^{*}]} \xrightarrow{\varepsilon} q_{([a^{*?}]), 0} \xrightarrow{\varepsilon} f_{([a^{*?}])} \xrightarrow{\varepsilon} q_{([a^{*?}]), 0} \xrightarrow{\varepsilon} q_{a, 0} \xrightarrow{\varepsilon} \\
 \hspace*{6mm} q_{a, 1} \xrightarrow{a} f_a \xrightarrow{\varepsilon} f_{([a^{*?}])} \xrightarrow{\varepsilon} q_{([a^{*?}]), 0} \xrightarrow{\varepsilon} f_{([a^{*?}])}  \xrightarrow{\varepsilon}  f_{[([a^{*?}])^*]},
\end{array}
$$
where $x_{e} = a$ and $ x_{([a^{*?}])} = \varepsilon$, namely, $e $ is matched to ``$a$''  and  $([a^{*?}])$ is matched to $\varepsilon$. Therefore, the semantics of $e$ defined by $\cT^\prime_e$  is \emph{inconsistent} with semantics of /($a$*?)*/ in node.js. Intuitively, the semantics of /($a$*?)*/ in node.js requires that either it is matched to $\varepsilon$ in whole and the subexpression $a$*? is \emph{not} matched at all, or it is matched to a concatenation of \emph{non-empty} strings where each of them matches $a$*?. This semantics can be captured by splitting the set of final states of PSSTs into two disjoint subsets.
%}
%
	\begin{figure}[tb]
%		\vspace{-2mm}
