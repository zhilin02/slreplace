...
\end{array}
$$
where $x_{e} = aaa$ and $ x_{([a^{*?}])} = \varepsilon$, namely, $e $ is matched to ``$a$''  and  $([a^{*?}])$ is matched to $\varepsilon$. Therefore, the semantics of $e$ defined by $\cT^\prime_e$  is \emph{inconsistent} with semantics of /($a$*?)*/ in node.js. Intuitively, the semantics of /($a$*?)*/ in node.js requires that either it is matched to $\varepsilon$ in whole and the subexpression $a$*? is \emph{not} matched at all, or it is matched to a concatenation of \emph{non-empty} strings where each of them matches $a$*?. This semantics can be captured by splitting the set of final states of PSSTs into two disjoint subsets.
%
	\begin{figure}[tb]
%		\vspace{-2mm}
		\centering
