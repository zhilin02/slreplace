...
For enable symbolic execution of the JavaScript programs like in Fig.~\ref{fig-run-exmp-path}, one needs to model both the greedy semantics of the Kleene star and store the matches of capturing groups. For this purpose, we introduce prioritized streaming string transducers (PSST, cf.\ Section~\ref{sec-rwre}) by which {\sf replace(nameReg, ``$\$$2, $\$$1'')} is captured as a PSST $\cT$, where the \emph{priorities} are used to model the greedy semantics of $*$ and the \emph{string variables} are used to record the matches of the capturing groups as well as the return value. Then the  symbolic execution of the program in Fig.~\ref{fig-run-exmp-path} can be equivalently turned into the satisfiability of the following string constraint,
%
\[{\sf authorList} \in \cA_{\sf autListReg}\wedge {\sf result} = \cT({\sf authorList}) \wedge {\sf result} \in \cA_{\sf postConReg},\]
%
where {\sf postConReg} = \mintinline{javascript}{/^.*\sand[^,]*\sand.*$/}, and $\cA_{\sf autListReg}, \cA_{\sf postConReg}$ are the finite-state automata corresponding {\sf autListReg} and {\sf postConReg} respectively. 

\zhilin{@Philip, please check}
Then we apply the proof rules in Section~\ref{sect:calculus} to solve the string constraint, in the following three phases. The string constraint solving relies on the fact that the pre-images of regular languages under PSSTs are regular (see Lemma~\ref{lem:psst_preimage}).
%
\begin{itemize}
\item \emph{Phase 1:} Apply the Boolean rules (first row of
  Table~\ref{tab:calculus}) to eliminate Boolean operators and obtain the sequent 
  %
  \[
  {\sf authorList} \in \cA_{\sf autListReg}, {\sf result} = \cT({\sf authorList}), {\sf result} \in \cA_{\sf postConReg}.
  \]
%
\item \emph{Phase 2:} We compute the pre-image of the regular language defined by $\cA_{\sf postConReg}$ under $\cT$, which is a finite-state automaton $\cB$, apply the proof rule {\ruleName{Bwd-Prop}}, and obtain the following sequent,
%
\[{\sf authorList} \in \cA_{\sf autListReg}, {\sf result} = \cT({\sf authorList}),  {\sf result} \in \cA_{\sf postConReg}, {\sf authorList} \in \cB.\]
%
\item \emph{Phase 3:} Since ${\sf authorList}$ is an input string variable, namely,  the
  string variable that does not occur as left-hand side of equations or
  function applications,  we apply rule~\ruleName{Cut} for ${\sf authorList}$ and 
 introduce an assignment~${\sf authorList} \in w$ that is consistent with the
  regex constraints on ${\sf authorList}$, i.e ${\sf authorList} \in \cA_{\sf autListReg}$ and ${\sf authorList} \in \cB$. 
  Then systematically apply
  \ruleName{Subsume} and \ruleName{Fwd-Prop-Elim} to
  evaluate all remaining formulas and turn the proof goal into a
  solution, thus showing the original string constraint is satisfiable.
\end{itemize}
