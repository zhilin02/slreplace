...
\noindent{\bf Lemma~\ref{lem-str-fun-to-psst}}.
    The satisfiability of $\strline$ reduces to the satisfiability of the string constraints that are a boolean combinations of formulas of the form $z=x \concat y$, $y=\cT(x)$, and $x \in \cA$, where $\cT$ is a PSST and $\cA$ is an FA.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        Next, we show how to construct the PSST for $\replaceall_{\pat, \rep}$ where all the references in $\rep$ are of the form $\$i$.
        ...
        If we tried to use a single variable – e.g. $\rep[x/\$i_1,x/\$i_2]$ – then the resulting transition in the encoding below would not be copyless.
         ...
         %For every $j \in [k]$, let $e'_{i_j}$ be the subexpression of $\pat$ corresponding to the $i_j$th capturing group, and $\mathscr{S}_{e'_{i_j}}[\cT_\pat]$ be the set of isomorphic copies of $\cT_{e'_{i_j}}$ in $\cT_\pat$. (Recall that in the construction of $\cT_e$ in Section~\ref{sec-rwre}, there may be multiple copies of $\cT_{e'_{i_j}}$ for a subexpression $e'_{i_j}$.)
%
   Then $\cT_{\replaceall_{\pat,\rep}}$ is obtained from $\cT_\pat$ by adding a fresh state $q'_0$ such that (see Fig.~\ref{fig-psst-replaceall})
        ...
        Observe that the PSST will be copyless: in all but the last case, variable assignments are of the form $x_{e'} = x_{e'} a$, $x_{e'} = x_{e'}$, $x_{e'} = \varepsilon$, or $x_{e'} = \nullchar$;
        ...
        Formally, $\cT_{\replaceall_{\pat, \rep}} = (Q_\pat \cup \{q'_0\}$, $\Sigma$, $X'$, $\delta'$, $\tau', E', q'_0, F')$ where
        \begin{itemize}
            \item $q'_0 \not \in Q_\pat$,
