...
\philipp{why not add negation and disjunctions as well?}
\[
\begin{array}{l c l}
\smallskip
...
& & y = \replaceall_{\pat, \rep}(x)   \mid
 x \in e \mid \varphi \wedge \varphi \mid \varphi \vee \varphi\
\label{eq:SL}
\end{array}
\]
where
...
A $\strline$ formula is said to be \emph{straight-line}, if 1) each variable $x$ occurs in the left-hand side of equations at most once; and 2) if $x$ occurs in the left-hand side of some equation, then all its occurrences on the right-hand sides of the equations or in assertions are after the equation.
