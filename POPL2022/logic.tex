...
The formal semantics of $\extract_{i, e}(x)$ is defined by the PSST $\cT_e$ constructed from $e$ in Section~\ref{sec-rwre}, with all the string variables, except the string variable corresponding to the $i$-th capturing group, removed.
%
...
A $\strline$ formula is said to be \emph{straight-line}, if 1) it contains neither negation nor disjunction, 2) each variable $x$ occurs in the left-hand side of equations at most once; and 3) the equations can be ordered into a sequence, say $x_1 = t_1, \ldots, x_n = t_n$, such that for each variable $x$, if $x = x_i$, then $x$ does not occur in $t_1, \ldots, t_{i-1}$. Let $\strlinesl$ denote the set of straight-line $\strline$ formulas.
