...
Nevertheless, regular expressions in programming languages ({\regexp}) such as JavaScript, feature non-standard semantics of operators 
...
%% contribution
In this paper, we advocate that string solvers should natively support
important features of \regexp{} and propose such an approach. 
%of natively supporting these features of REs in string constraint solving. 
The approach relies on a string constraint language including \regexp{}-dependent string functions such as extract and replace(all). 
%
The key idea of our approach is to introduce a new automata model, called \emph{prioritized streaming string transducers} (PSST), to formalize the semantics of \regexp{}-dependent string functions.
...
model capturing groups. We carry out extensive empirical studies
to validate our formalization of \regexp{}-dependent functions as {\PSST}s with respect to JavaScript
semantics.
%
...
The approach solves the string constraints by using a sound sequent calculus that exploits nice closure and algorithmic properties of
{\PSST}s, and performs a kind of propagation of regular constraints by means of 
taking post-images or pre-images. %, as well as to measure the performance of our solver., 
In particular, we show that {\PSST}s preserve regularity, i.e. pre-images of regular constraints under {\PSST}s are regular. 
Although the satisfiability of the constraint language is generally 
undecidable, we show that our approach is complete for the so-called straight-line fragment.
%
We evaluate its performance on over 195\,000 string constraints generated from an open-source {\regexp} library. The experimental results show the efficacy
of our approach, drastically improving the existing methods (via symbolic execution) in  both precision and efficiency.
