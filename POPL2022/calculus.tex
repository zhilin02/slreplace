...
\begin{table}
  ...
  \begin{gather*}
    ...
    \begin{array}{cl}
      ...
      &
        \philipp{also \ruleName{=-Prop-Elim}?}
      \\[3ex]
      \infer[$\not=$-Prop-Elim]
      {\seqq{x \in e, x \not= y}}
...
concatenation, extraction, and replacement, but it can be extended by
other functions that can be represented as PSSTs.
...
The next three rules handle equations between string
variables. Rule~\ruleName{=-Prop} propagates regex constraints from
the left-hand side to the right-hand side of an equation;
\ruleName{$\not$=-Prop-Elim} similarly turns a singleton regex for the
left-hand side of a disequality into a regex constraint on the
right-hand side. As a side-effect, the latter rule also removes the
disequality, which is implied by the resulting two regex constraints.
As a convention, we allow application of \ruleName{=-Prop} and
...
adds a regex constraint~$x \in e$ on the value of a function derived
from constraints on the arguments. The regex~$e$ encodes the image of
the argument regexes under $f$:
%
...
easy to see, however, that the image of regular languages under the
...
propagation, and derives regex constraints about function arguments
from a constraint on the function value. The argument constraints
encode the \emph{pre-image} of the result language:
...
)$, i.e., they are \emph{recognizable languages.} This implies that
