...
When $\refbefore$ and $\refafter$ are not used, the PSSTs in the reduction are copyless, which means the exponential blow-up in the size of the input \FA{} $\Aut$ can be avoided. This means that the exponentials do not stack on top of each other during the backwards analysis and the non-elementary blow-up is not necessary. \zhilin{I think we should be careful  about the complexity here, since the current transformation from Regexes to PSSTs with two disjoint subsets of final states does incur an exponential blow-up (See Section 3).}
