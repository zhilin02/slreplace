...
Applied string operations range from concatenation, length, substring, to complex functions 
such as 
%the functions dependent on regular expressions 
match, replace, split, parseInt, 
and character encoding/decoding.  As a result, string-manipulating programs are
...
Regular expression is not only a classical concept in formal language theory (see e.g. \cite{HU79}), but also an indispensable part of the string library
in programming languages \cite{Berkeley-JavaScript,BM17,LMK19,HAMPI}.
%
Many important string functions in programming languages, such as exec, test, search, match, replace, and split in Javascript and match, findall, search, sub, and split in Python, involve regular expressions. 
...
%in programming languages \cite{Berkeley-JavaScript,BM17,LMK19,HAMPI}.
%
Nevertheless, regular expressions in programming languages are dramatically different from 
classical regular expressions in formal language theory. 
...
the other hand, regular expressions in programming languages differ from classical ones mainly in the following two 
...
Regular expressions in programming languages are in general more expressive than classical ones, e.g., it is known that
...
Regular-expression matching is the core of string functions involving regular expressions in programming languages. For instance, in Javascript, string functions exec, test, match, and search are all variants of regular-expression matching, moreover, the functions replace and split recursively call regular-expression matching as subprocedures. 
Nevertheless, regular-expression matching in programming languages are dramatically different from that in formal language theory, in the following two aspects.
...
\begin{enumerate}
\item In programming languages, regular expressions are not required to be matched to the whole string, but to a substring, which intuitively corresponds to the first match of the regular expression in the string, moreover, the non-standard semantics of operators guarantee that this matching is \emph{deterministic} in the sense that for a given regular expression and a string, the matching returns a \emph{unique} substring (if there is any). For instance, the matching of $e=$\mintinline{javascript}{(\d+)(\d*)} to $ab123c$ returns $123$, instead of $1$ or $12$.
%
\item Regular expressions in programming languages typically contain capturing groups, and the matchings of these capturing groups in strings should also be returned, moreover, these matchings are also deterministic. For instance, the matching of $e=$\mintinline{javascript}{(\d+)(\d*)} to $ab123c$ actually returns an array of strings [``123'', ``123'', ``''], where the first entry is the matching of  $e$, and the last two entries are the matchings of the two capturing groups.
\end{enumerate}

As a result of the fundamental role of regular expressions in string processing, almost all state-of-the-art string constraint solvers (e.g.
Z3, CVC4, Z3-str/2/3/4, ABC, Norn, Trau, OSTRICH, S2S, Qzy, Stranger, Sloth, Slog, Slent, Gecode+S, G-Strings, HAMPI) %... \anthony{make sure we add all}) 
 support regular expressions. Nevertheless, what they support is only \emph{classical} regular expressions and their matching, aka membership constraints, i.e. whether a string belongs to the language defined by a regular expression, as we know it from formal language theory.  
...
Since the state-of-the-art string solvers support only the matchings of regular expressions and their matchings,
% instead of RWREs,
...
string-manipulating programs with apply workarounds.
...
%operator. 
%On the other hand, 
Both Aratha and \expose{} attempt to exploit string 
...
\begin{description}
    \item[(Q1)] Define the formal semantics of regular-expression matchings in programming languages, where at least the non-standard semantics of operators and capturing groups are covered.
    %
    \item[(Q2)] Incorporate string functions involving regular expressions in programming languages (e.g. match and replace functions)
