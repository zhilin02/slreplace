...
Our string solver is based on a formalization of the semantics of the $\extract$, $\replace$, and $\replaceall$ functions.
%
...
We show that $\extract$, $\replace$, and $\replaceall$ can all be expressed as PSSTs, thus providing formal semantics for these functions.
%
More importantly, we 
...
Moreover, our string solver relies on a sound sequent calculus (implemented in the 
...
regular-constraint-preserving. Since the $post$-image is not always regular-constraint-preserving,
we only propagate by taking $post$-image when this is regular-constraint-preserving.
On the other hand, one of our crucial results is that that taking $pre$-image 
is always regular-constraint-preserving. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\OMIT{
To this end, we introduce a new 
transducer model called \emph{Prioritized Streaming String Transducer (PSST)},
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
More precisely, we show that regular constraints are \emph{effectively closed under
