...
%    simultaneously involves both match and replace. 
    ...
    function in JavaScript; a more detailed example can be found
    in Section \ref{sec:mot}. Consider the snippet
    ...
    \texttt{newAuthorList} are capped to 20. In this case, we would want to
    check if the string \texttt{newAuthorList} could contain
    more than 19 \texttt{';'}, assuming that \texttt{authorList} contains at most
    than 19 \texttt{';'}.
    %\qed
\end{example}
...
monumental theoretical and programming task, although this sometimes comes at a 
cost of precision and performance. Secondly, we present \emph{the first} string
theories and string solvers that provide such a support.
