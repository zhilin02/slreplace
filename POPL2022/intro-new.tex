...
execution}, possibly in combination with dynamic analysis. This technique 
analyses a static path in a program, by viewing it as a constraint $\phi$, 
whose feasibility can be checked by constraint solvers. This has motivated the
highly active research area of \emph{string solving}, resulting in the
development of numerous string solvers in the last decade or so including
\cite{???}.

One highly challenging problem in the development of string solvers is the need
...
functions (in particular, concatenation, regular constraints, and length
constraints). For example, the importance of supporting functions like
replaceAll function (i.e. replace with global flag) was explained in \cite{??};
ever since, quite a number of string solvers support this operator.
Unfortunately, this is far from enough because actual string manipulations
performed in programming languages depend on Perl-style regular expressions,
whose syntaxes and semantics are quite different from the notion of regular
expressions in formal language theory \cite{HU79}.
Indeed,
...
search, sub, and split in Python, depend on Perl-style regular expressions. 
