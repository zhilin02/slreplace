...
to the extent that the semantics of regular expressions\philipp{not sure what this is supposed to tell} is used and that 
RegEx features like capture groups are not allowed. This limitation of existing
string solvers was already mentioned in recent papers \cite{LMK19}.
...
\cite{Z3}. Unfortunately, this mapping is an \emph{approximation} at best\philipp{remove ``at best'', it is an approximation!}, since
such complex string manipulations are generally \emph{inexpressible} in any 
existing string theories\philipp{too general, just say that they are not expressible in the SMT-LIB theory?}. To leverage this, CEGAR (counter-example guided 
...
(e.g. the aforementioned examples). \anthony{Zhilin: please check this}\zhilin{The description is consistent with the experimental results.}
...
(at least at some levels\philipp{should be reformulated. current solvers support RegEx \emph{on some level}, namely excluding look-arounds, capture groups, etc.}) in their SMT theories. Existing work
(e.g. the reduction to Z3 provided by \cite{LMK19}) shows that this is a 
monumental theoretical and programming task, although\philipp{``and in addition''?} this can come at a 
...
\cite{LMK19}) by 30x--50x. We provide more details of our contributions
\philipp{agreed. we should also say what parameter 30x-50x refers to} We provide more details of our contributions
below.
...
also cover the most important features of \regexp{} (including greedy/lazy
matching, capture groups, among others) that make up 74.97\% of the benchmarks 
from
\cite{LMK19}. More importantly, we have performed an extensive experiment
...
Our string solver is based on a sound sequent calculus (implemented in the 
standard DPLL(T) setting of SMT solvers~\cite{NieuwenhuisetalJACM2006}) 
%Our calculus is sound for the
that attempts to 
(1) \emph{propagate} \regexp{} constraints in the formula around by means of
the string functions $\cdot$, $\replace$, $\replaceall$, and $\extract$, and 
(2) either detects conflicting constraints, or find a satisfiable assignment.
...
$x \in S$ and \regexp{} $e$.
A single step of the \regexp{}-constraint propagation computes either the 
...
it is crucial that each step of our constraint propagation is 
\regexp{}-preserving. Since the $post$-image is not always \regexp{}-preserving,
we only propagate by taking $post$-image when this is \regexp{}-preserving.
On the other hand, one of our crucial results is that that taking $pre$-image 
is always \regexp{}-preserving. To this end, we introduce a new 
...
Crucially, we show that \regexp{}-constraints are \emph{effectively closed under
...
\anthony{This is the end of my intro for now; below can be erased as needed}
