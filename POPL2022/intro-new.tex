...
(e.g. Example~\ref{exmp-name-swap}). \anthony{Zhilin: please check this}\zhilin{Expose does not terminate when replace function with global flag is used.}
...
\zhilin{I think we should say why the current string solvers do not support Regexes, what are the difficulties (e.g. defining the Regex-string matching semantics formally)? How we will deal with the difficulties in the contribution below.}
...
\cite{aratha,LMK19}) by 30x-50x. \zhilin{I think Aratha should be removed here. We do not compare with Aratha, since it is unable to deal with Regexes.} We provide more details of our contributions
below.
...
matching, capture groups, among others) that make up xx\% of the benchmarks from
...
Our string solver is based on a first-order sequent calculus ...
\anthony{Philipp, maybe you want to say something here?}
