...
\mat{presumably it's important that the state space does not change?}\zhilin{not really, since for $\varepsilon \not \in \Ll(e_0)$, the state space will be increased anyway}
%
...
\item $\cA_{e_0}$ has two modes, ``$\searchleft$'' and ``$\searchlong$'', which intuitively means search for the first and last position of the leftmost and longest matching respectively,
 %
    ...
    \mat{How can an automaton start a thread?  Is the vector of sets of reachable states unbounded in length?}\zhilin{the description is similar to that for alternating automata: one can think multiple threads are running in parallel, each thread responsible for one state. the vector of sets of reachable states will be made bounded by applying the trick specified below.} 
