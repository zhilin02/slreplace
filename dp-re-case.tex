...
\begin{itemize}
	...
	\begin{itemize}
		...
		$\left((\rho, \searchleft, S), a, \left(\red(\delta_0(\rho\{q_{0,0}\}, a)) \{q_{0,0}\}, \searchleft, \delta_0(S,a) \right) \right) \in \delta_{e_0},$
		...
		%
		\item suppose $(\rho \{q_{0,0}\}, \searchleft, S) \in Q_{e_0}$ such that $\rho = S_1 \cdots S_n$ with $n \ge 0$, $a \in \Sigma$, and $\big(\bigcup \limits_{j \in [n]} \delta_0(S_j, a) \big) \cap F_0 = \emptyset$, moreover, $\delta_0(S,a) \cap F_0 = \emptyset$, then
...
\begin{example}
Let $e_0 = 0^*0 11^* + 0^*10^*$. Then $\cA_{0}$ and $\cA_{e_0}$ are illustrated in Figure. 
\end{example}
