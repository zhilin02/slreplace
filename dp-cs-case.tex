...
\mat{In the below, do you mean $v \in (\Sigma^\ast u \Sigma^\ast)^\ast$?  I'm surprised that the repetition of $u$ is not specified somehow.}
\mat{Also below, should $u[1]\dots u[k-1]$ be $u[j] \dots u[k-1]$ (this appears in the def of the parsing automaton too)?}
\mat{Finally, below, can $u$ appear in $v_j$ for $j < l + 1$?}
...
\mat{Don't you want to keep the states since you use them below?}
...
\mat{Also need to say the same about $z$?}
