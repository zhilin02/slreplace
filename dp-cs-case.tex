...
$$
Let $T_z = \{(q_i, q'_i) \mid i \in [n]\}$. Then $w' \in \Ll(\cA_3) \cap\ \bigcap \limits_{(q,q') \in T_z} \Ll(\cA_1(q, q'))$. Therefore, $\Ll(\cA_3) \cap\ \bigcap \limits_{(q,q') \in T_z} \Ll(\cA_1(q, q')) \neq \emptyset$. Similar to the single letter case, in the following, we will construct an NFA $\cB_{\cA_1, u, T_z}$ to characterise the satisfiability of $C$.  More precisely, $C$ is satisfiable iff there is $T_z \subseteq Q_1 \times Q_1$ such that $\Ll(\cA_3) \cap\ \bigcap \limits_{(q,q') \in T_z} \Ll(\cA_1(q, q')) \neq \emptyset$ and
...
\mat{In the below, do you mean $v \in (\Sigma^\ast u \Sigma^\ast)^\ast$?  I'm surprised that the repetition of $u$ is not specified somehow.}\zhilin{($\Sigma^\ast u \Sigma^\ast)^\ast = \{\varepsilon\} \cup \Sigma^\ast u \Sigma^\ast$. Here I just wan to say that at least one occurrence of $u$ occurs.}
\mat{Also below, should $u[1]\dots u[k-1]$ be $u[j] \dots u[k-1]$ (this appears in the def of the parsing automaton too)?}
\mat{Finally, below, can $u$ appear in $v_j$ for $j < l + 1$?}\zhilin{$v_j u[1] \dots u[k-1] \not \in \Sigma^\ast u \Sigma^\ast$ implies that $u$ does not occur in $v_j$. $u[1] \cdots u[k-1]$ is the maximal strict prefix of $u$. $v_j u[1] \dots u[k-1] \not \in \Sigma^\ast u \Sigma^\ast$ says that $u$ does not appear in the strict prefixes of $v_j u$.}
...
Let $v = v_1 u v_2 u \cdots v_l u v_{l+1}$ such that $v_j u[1] \dots u[k-1] \not \in \Sigma^\ast u \Sigma^\ast$ for each $1 \le j \le l$, in addition, $v_{l+1} \not \in \Sigma^\ast u \Sigma^\ast$. Then there exists a \emph{unique} accepting run $r$ of $\cA_u$ on $v$ such that the state sequence in $r$ is of the form
$q_0\ r_1\ q_0\ r_2\ q_0\ \cdots\ r_l\ q_0\ r_{l+1}$, where for each $j \in [l]$, $r_j \in (Q_{\search})^+ \concat Q_{\verify, 1} \concat \cdots  \concat Q_{\verify, k-1}$, and $r_{l+1} \in (Q_{\search})^*$. Intuitively, each occurence of $q_0$, except the first one, witnesses the \emph{first} occurrence of $u$ from the beginning or after its previous occurrence.
...
\mat{Don't you want to keep the states since you use them below?}\zhilin{you are right. I mean removing the transitions associated with them.}
...
In each step of the reduction, suppose the two edges out of $x$ are currently removed, let the two edges be from $x$ to $y$ and $z$ and labeled by $({\sf l}, u)$ and $({\sf r}, u)$ respectively, then each element of $(\cT, \cP)$ of $\cE_i(x)$ may be transformed into an element $(\cT', \cP')$ of $\cE_{i+1}(y)$ such that $|\cT'| = O(|u||\cT|)$, meanwhile, it may also be transformed into an element $(\cT'', \cP'')$ of $\cE_{i+1}(z)$ such that $\cT''$ has the same state space as $\cT$.
%\mat{Also need to say the same about $z$?}\zhilin{the state space of the regular constraint for $z$ is not changed.}
Thus, after the reduction, for each source variable, there may be exponentially many regular constraints generated, and each of them may have a state space of at most exponential size. To solve the nonemptiness problem of the intersection of all these regular constraints, the exponential space is sufficient. Consequently, in this case, we still obtain a EXPSPACE upper bound. In addition, if $\lftlen(G_C)$, the $\rpleft$-length of $G_C$, is bounded by a constant $c$, then $\dmdidx(G_C)$ is also bounded by $c$, therefore, according to Proposition~\ref{prop-di}, there are at most polynomially different paths in $G_C$, we deduce that for each source variable $x$, $\cE(x)$ contains at most polynomially many elements. In addition, since the number of $\rpleft$-edges in each path is bounded by $c$, during the execution of the decision procedure, the number of times when $|\cT'| = O(|u||\cT|)$ happens is bounded by $c$. Therefore, for each source variable $x$ and each element $(\cT'', \cP'')$ in $\cE(x)$,  $|\cT''|$ is at most polynomial in the size of $C$. We then conclude that for each source variable $x$, the nonemptiness of the intersection of all the regular constraints in $\cE(x)$ can be solved in polynomial space. In this case, we obtain a PSPACE upper bound.

\mat{One might object that this bit is too sketchy, though perhaps an attempt to be formal will be tedious and repetitive.}\zhilin{polished, still sketchy, please have a look. }
