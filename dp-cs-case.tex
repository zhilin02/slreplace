...
From $u$, we will construct a parsing automaton $\cA_u$ which parses a string $v$ containing at least one instance of $u$ (i.e.\ $v \in \Sigma^\ast u \Sigma^\ast$) into $v_1 u v_2 u \dots v_l u v_{l+1}$ such that $v_j u[1] \dots u[k-1] \not \in \Sigma^\ast u \Sigma^\ast$ for each $1 \le j \le l$.
This ensures that the only (and first) instance of $u$ in each $v_j u$ is the suffix.
Finally, we also require $v_{l+1} \not \in \Sigma^\ast u \Sigma^\ast$.
The $k$-window profiles w.r.t. $u$ will be used by the parsing automaton to ensure that it does not miss any occurrences of $u$.
