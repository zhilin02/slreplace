...
\begin{proposition}
The number of $k$-window profiles w.r.t. $u$ is exactly $k$.
\end{proposition}
\begin{proof}
...
We are ready to present the construction of $\cB_{\cA_1, u,  T_{z}}$. The NFA $\cB_{\cA_1, u,  T_{z}}$ is constructed by the following two-step procedure.
\begin{enumerate}
\item Construct the product automaton of $\cA_1$ and $\cA_u$. Then remove all the states from $Q_1 \times Q_{\verify}$ as well as the transitions associated with them.
