...
which belongs to all fragments in Corollary~\ref{cor-pspace}.\zhilin{Remark for the pspace lower bound, please check.}
%
At last, we would like to remark that the restrictions in Corollary~\ref{cor-pspace} are partially inspired by the  benchmarks in practice. Diamond indices (intuitively, the ``nesting depth'' of $\replaceall(x,a,x)$) are likely to be small in practice because the constraints like $\replaceall(x,a,x)$ are rather artificial and rarely occur in practice. Moreover, the $l$-length reflects the nesting depth of replaceall w.r.t. the first parameter, which is also likely to be small. Finally, for string constraints with concatenation and $\replaceall$ where pattern/replacement parameters are constants, the diamond index is no greater than the ``dimension'' defined in \cite{LB16}, where it was shown that for such string constraints existing benchmarks mostly have dimensions at most three. \zhilin{In the rebuttal, we stated that diamond index and dimension for constraints which use only concatenation. But I think the statement is wrong. Please check.}
