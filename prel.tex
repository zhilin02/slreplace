...
The term DAG stands for \emph{directed acylic graphs}. \zhilin{maybe extended by including more graph-theoretical notations.}
...
\begin{example}
For instance, $\replaceall(abac, a^*, d) =d a d b d a d c d$ and $\replaceall(abac, a^+, d)=dbdc$.
\end{example}
...
The satisfiability problem of $\strline[\replaceall]$ is undecidable, if the second parameters of the $\replaceall$ terms are regular expressions.
\end{theorem}
