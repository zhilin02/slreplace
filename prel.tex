...
\mat{If we cite this, don't we also need that NFA are closed under boolean operations, since we use it often?}\zhilin{added}
...
In this paper, we focus on the satisfiability problem of $\strline[\concat,\replaceall]$, which is formalised as follows. 
...
To approach this problem, we identify several fragments of  $\strline[\concat,\replaceall]$, depending on whether the second and the third parameters are constants or variables.  We shall investigate extensively the satisfiability problem of the fragments of $\strline[\concat, \replaceall]$ (see Table~\ref{tab-sum}).  Note that for $x=\replaceall (y, p, z)$, $p$ is referred to as a \emph{pattern} and $z$ is referred to as a \emph{replacement}.
