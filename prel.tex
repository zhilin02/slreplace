...
\begin{proof}
	We reduce from the Post Correspondence Problem (PCP). The input of the problem consists of two finite lists $\alpha_{1},\ldots ,\alpha_{N}$ and $\beta_1,\ldots ,\beta_N$ of nonempty strings over $\Sigma$. A solution to this problem is a sequence of indices $(i_{k})_{1\leq k\leq K}$ with $ K\geq 1$ and $ 1\leq i_{k}\leq N$ for all $k$, such that
...
$
	...
	Without loss of generality, suppose $\Sigma \cap \{1,\cdots, N\} = \emptyset$ and $\$ \not \in \Sigma \cup \{1,\cdots, N\}$. Let $\Sigma' = \Sigma \cup \{1,\dots, N\} \cup \{\$\}$. In the following, we will construct a $\strline[\replaceall]$ formula $C$ over $\Sigma'$ such that the PCP instance has a solution iff $C$ is satisfiable. To fulfill the reduction, the formula $C$ utilises the capability that the second parameter of $\replaceall$ terms are variables.
...
Towards a complete solution of $\strline[\concat, \replaceall]$, let us start with a simple observation, which demonstrates the power of the $\replaceall$ function.

%In the definition of $\strline[\replaceall]$, we observe that
%
