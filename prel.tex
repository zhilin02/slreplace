...
\begin{example}
Let $G_C$ be the dependency graph illustrated in Figure~\ref{fig-dmdidx-exmp}. It is easy to see that $\dmdidx(G_C)$ is $3$. In addition, there are $2^3=8$ paths form $x_1$ to $x_4$. If we generalise $G_C$ in Figure~\ref{fig-dmdidx-exmp} to a dependency graph comprising $n$ diamonds from $x_1$ to $x_2$, $\cdots$, from $x_n$ to $x_{n+1}$ respectively, then the diamond index of the resulting dependency graph is $n$ and there are $2^n$ paths from $x_1$ to $x_{n+1}$ in the graph.
\begin{figure}[htbp]
\begin{center}
...
In Section~\ref{sec:replaceallsl}--\ref{sec:replaceallre}, we will also apply a refined analysis of the complexity of the decision procedures and get the following results.
...
\begin{itemize}
\item the class of $\strline[\replaceall]$ formulae where the second parameters of all $\replaceall$ terms are single letters and the diamond indices of the dependency graphs are bounded by a constant $c$, 
%
\item the class of $\strline[\replaceall]$ formulae where the second parameters of all $\replaceall$ terms are constant strings and the $\rpleft$-lengths of the dependency graphs are bounded by a constant $c$, 

%
\item the class of $\strline[\replaceall]$ formulae where the $\rpleft$-lengths of the dependency graphs are at most $1$.
\end{itemize}
\end{corollary}
