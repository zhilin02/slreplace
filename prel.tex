...
%
%
We assume a countably set of variables, of the data type $\str$. We will use $x, y, z, \cdots$ to denote the variables of data type $\str$. %, and $\mathfrak{l}, \mathfrak{m}, \mathfrak{n}, \cdots$ to denote the variables of data type $\intnum$. \tl{also shall we introduce the type for regular expressions? I replaced x by X in def 3.4 as I feel that they should be distinguished.}
...
For a formula $\varphi$, let $\vars(\varphi)$ denote the set of variables occurring in $\varphi$. Similarly for $\vars(\psi)$. Given a relational constraint $\varphi$, a variable $x$ is called a \emph{source variable} of $\varphi$ if $\varphi$ \emph{does not} contain a conjunct of the form $x = s_1 \concat s_2$ or $x = \replaceall(\dots)$.
