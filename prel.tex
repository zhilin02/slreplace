...
Let $\mathbb{Z}$ and $\Nat$ denote the set of integers and natural numbers respectively. For $k \in \Nat$, let $[k] = \{1,\cdots, k\}$. For a vector $\vec{x}=(x_1,\cdots, x_n)$, let $|\vec{x}|$ denote the length of $\vec{x}$ (i.e., $n$), $\vec{x}[i]$ denote $x_i$ for each $i \in [n]$, and %, . For a vector $\vec{x} = (x_1, \cdots, x_n)$, let 
