...
If $|u|=1$, then $\cA_u$ is trivial, that is, $\cA_u$ comprises the self-loops $q_0 \xrightarrow{\sigma} q_0$ for each $\sigma \in \Sigma$.

In the following, we assume that $|u| = k \ge 2$.
...
Let $u = u_1\dots u_k$ such that $u_i \in \Sigma$ for each $i \in [k]$.  A $k$-\emph{window profile} $\vec{W}$ w.r.t. $u$ is an element of $\{\bot,\top\}^{k}$. Intuitively, in the position $i$ of a string $v$, $\vec{W}$ is an abstraction of the substring $v_{i-k+1} \dots v_i$ such that for each $j \in [k]$, $\vec{W}(j) = \top$ iff $v_{i-j+1} \dots v_i = u_1 \dots u_i$. Let $\wprof_{u, k}$ denote the set of $k$-window profiles w.r.t. $u$.
\begin{itemize}
\item  $Q_u = \{(\search, \vec{W}) \mid \vec{W} \in \wprof_{u, k} \} \cup \{(\verify, j, \vec{W}) \mid 1 \le j \le k, \vec{W} \in \wprof_{u,k}\}$, where $\search$ and $\verify$ are used to denote whether $\cA_u$ is in the ``search''-mode to search the next occurrence of $u$, or in the ``verify'' mode to verify that the current position is a part of an occurrence of $u$, 
%
...
\begin{itemize}
\item $(\search, \vec{W}) \xrightarrow{\sigma} (\search, \vec{W}')$, where $\vec{W}'(1) = \top$ iff $\sigma = u_1$, and for each $i: 2 \le i \le k-1$, $\vec{W}'(i) =\top$ iff $\vec{W}({i-1}) = \top$ and $\sigma = u_{i}$, in addition, $\vec{W}'(k)=\bot$,
%
\item if $\sigma = u_1$, then $(\search, \vec{W}) \xrightarrow{\sigma} (\verify, 1, \vec{W}')$,  where $\vec{W}'(1)=\top$,  and for each $i: 2 \le i \le k-1$, $\vec{W}'(i) =\top$ iff $\vec{W}({i-1}) = \top$ and $\sigma = u_{i}$, in addition, $\vec{W}'(k)=\bot$,
%
\end{itemize}
%
\item for each state $(\verify, i-1, \vec{W})$ such that $2 \le i \le k-1$, $\sigma = u_{i}$, and $\vec{W}(k-1)=\bot$ or $\sigma \neq u_k$,  we have $(\verify, i-1, \vec{W}) \xrightarrow{\sigma} (\verify, i, \vec{W}')$, where for each $j: 1 \le j < i$, $\vec{W}'(j)=\bot$, $\vec{W}'(i)=\top$, and for each $j: i < j < k$, $\vec{W}'(j) = \top$ iff $\vec{W}(j-1)=\top$ and $\sigma = u_j$, in addition, $\vec{W}'(k)=\bot$, 
%
\item for each state $(\verify, k-1, \vec{W})$, $(\verify, k-1, \vec{W}) \xrightarrow{u_k} (\search, \bot^k)$. 
%
\end{itemize}
Note that the constraint $W(k-1) = \bot$ or $\sigma \neq u_k$ is used to guarantee that $v_j u_1 \dots u_{k-1} \not \in \Sigma^\ast u \Sigma^\ast$ for each $1 \le j \le l$.
%
\item $F_u=\{(\search, \vec{W}) \mid \vec{W} \in \wprof_{u, k} \} $.
\end{itemize}
We would like to remark that $\cA_u$ is \emph{unambiguous}, more specifically, on each word $w$, there is \emph{exactly one accepting run} of $\cA_u$ on $w$.
