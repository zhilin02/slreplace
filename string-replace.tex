...
\tl{I am not clear what do you mean by parsing. what does $A_u$ accept? It should be $v_1 u v_2 u \dots v_l u v_{l+1}$ with the property that $v_ju[1] \dots u[k-1]$ does not contain $u$ as substring?}\zhilin{the purpose of $\cA_u$ is to mimic the semantics of replacement function, without actual replacing, in other words, decompose $v$ into $v_1, u, v_2, u, \dots ,v_l, u, v_{l+1}$ satisfying the desired constraints.}
