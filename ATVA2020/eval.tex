...
{\slent}~\cite{WC+18} because it uses its own input
...
\ostrich+ consistently outperforms \ostrichi\ and \ostrichii, with the
exception of \kaluzabench, where \ostrichii\ can handle all problems
in SSA form. For \ostrichi, this is to be expected because most
benchmarks contain integer functions. For \ostrichii, it turns out
that the encoding of $\indexof$, $\substring$, and $\charat$ as word
equations usually leads to problems that are not in SSA form, and
therefore beyond the scope of \ostrich.
