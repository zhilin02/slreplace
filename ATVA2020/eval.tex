...
For instance, we encode the non-idempotence of $\NFT$ into the path feasibility of the {\slint} program $y:=\NFT(x); z:=\NFT(y); S_{y \neq z}$, where $y$ and $z$ are two fresh string variables, and $S_{y \neq z}$ is the {\slint} program encoding $y \neq z$ (see Appendix~\ref{appendix:slint-prog-ineq} for the details). We also include in {\transducerbench+} three instances %generated from the running example in Section~\ref{exmp:running}, 
generated from a program to sanitize URLs against XSS attacks (see Appendix~\ref{exmp:running} for the details), 
...
to obtain a working version of {\trauplus} by the deadline, possibly because {\trau} requires two separate versions of Z3 to run. In addition, the algorithm in~\cite{AbdullaA+19} focuses on length-preserving transducers, which means that {\transducerbench}+ is beyond the scope of \trauplus.
