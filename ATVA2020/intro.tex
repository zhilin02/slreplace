...
the solutions. SLOTH \cite{HJLRV18} computes a ``global'' 
representation of all
solutions as (alternating) finite transducers. Dealing with length requires one
...
(e.g. \cite{VSS05}), which would give us a triply-exponential-time algorithm.
Lin and Barcelo \cite{LB16} provided an double-exponential upper bound on the 
...
It uses \emph{recognizable relations} (a finite union of regular constraints)
...
between the different variables in the recognizable relations.
%\emph{provide an
