...
\paragraph*{\slogbench}
 ...
 is adapted from the SLOG benchmark suite used by the SLOG tool~\cite{fang-yu-circuits}, containing 3,511~instances about strings only.
...
is the most well-known set of string benchmarks in the literature, and
contains 47,284 problems with regular constraints, concatenation, and the $\length$ function. The 47,284 benchmarks include 28,032 satisfiable and 9,058 unsatisfiable problems in SSA form.
...
\end{tabular}
\end{center}
\caption{Experimental results on different benchmarks suites.  '--' means that the tool is not applicable to the benchmark suite, and 'inconclusive' means that a tool gave up, timed out, or crashed.}
...
Table~\ref{tab-experiment} summarizes the experimental results. We did not observe incorrect answers by any tool.
% (note that the ground-truth are known and have been verified \tl{check this}  

There are two additional state-of-the-art solvers we do not include in
the evaluation, {\slent} and {\trauplus}.  We exclude
...
the benchmarks, but this number rises to 94 when using a bigger
...
number of satisfiable instances (1,309). The {\slogbenchr} benchmark
is beyond the scope of {\zthreetrau} which does not support
$\replace$.

On the three \pyexbench\ suites, {\zthreetrau} consistently solves the
biggest number of problems, by some margin. \ostrich+ solves a similar
...
solvers, while also able to handle benchmarks that are beyond the
scope of other tools due to the combination of string operations (and
in particular transducer applications) and integer
...
included in our implementation is to use standard automata methods,
like in \ostrich\ and as opposed to CEFAs, for simpler problems like
the \kaluzabench\ benchmarks.
