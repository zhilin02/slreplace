...
The experiments are executed on a virtual machine with Inter Xeon Silver 4210 2.20GHz and 2.19GHz CPU (2-core) and 8GB main memory, running 64bit Ubuntu 18.04 LTS OS and Java 1.8, with the timeout period set as 30 seconds. The experimental results are summarised in Table~\ref{tab-experiment}.

{\zthreetrau} does not support replace/replaceall.

sloth does not deal with integer constraints, so we choose not to compare

Trau+: unable to run. It is an integration of Sloth and Trau.
