...
Recently, researchers have also explored the decidability of string theories with more complex string operations and integer data type. Lin and Barcelo showed that the straight-line string logic with concatenation, finite transducers, and regular constraints, is decidable, even when extended with length, letter-counting functions, as well as $\indexof$ and $\charat$ functions \cite{LB16}. On the other hand, Chen et al. showed that although the straight-line string logic with the general form of $\replaceall$ (where the replacement parameter can be string variables) is decidable, it will become undecidable when extended with any of length, or $\indexof$, or $\charat$ function \cite{CCH+18}. 
...
with respective dynamic symbolic execution engines, Saner \cite{Saner08}, Kaluza~\cite{Berkeley-JavaScript}, Hampi~\cite{HAMPI},  Sushi~\cite{sushi}, Z3-str/Z3-str2/Z3-str3~\cite{Z3-str,Z3-str2,Z3-str3}, {\cvc}~\cite{cvc4,ReynoldsWBBLT17}, Stranger~\cite{YABI14,Stranger},  S3 and S3P~\cite{S3,TCJ16}, ACO-Solver \cite{ThomeSBB17}, FAT~\cite{Abdulla17}, Trau/{\zthreetrau}/{\trauplus} \cite{AbdullaACDHRR18-trau,AbdullaA+19,Z3-trau}, 
%
SLOG \cite{fang-yu-circuits}, {\slent} \cite{WC+18}, Norn~\cite{Abdulla14}, {\sloth} \cite{HJLRV18}, and {\ostrich} \cite{CHL+19}.
 %Pex.  
%
%
