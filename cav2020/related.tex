...
(2) practical (but generally incomplete) string solvers.  We restrict our attention to the results and solvers for string constraints involving integer data type.
...
On the other hand, Chen et al. showed that although the straight-line string logic with the general form of $\replaceall$ (where the replacement parameter can be string variables) is decidable, it will become undecidable when extended with any of the length, $\indexof$, $\charat$ functions \cite{CCH+18}. 
