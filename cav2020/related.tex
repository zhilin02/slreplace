...
(2) practical string solvers which are generally incomplete.  %and  heuristics and implementation.

\smallskip 
\noindent\textbf{Theoretical Results}
%We have discussed in Section \ref{sec:intro} works on 
%String constraints with 
The theory of strings with concatenation has a long history, largely motivated by %. This research programme builds on
the question of solving satisfiability of \emph{word equations}, i.e., a string equation containing concatenation of string constants and variables. Makanin showed decidability \cite{Makanin}. The upper bound was improved to PSPACE in \cite{P04} and  %using a word compression technique. 
a simpler algorithm was proposed in \cite{J17}. %using the recompression technique. 
...
The decidability (the PSPACE upper bound in this case) stands %can be retained 
...
The straight-line logic of \cite{LB16} unified the earlier logics by allowing concatenation, regular constraints, rational
...
It was shown in \cite{LB16} that the length constraints (in fact, letter-counting) can be
...
We mention some interesting benchmarks that are available for  string constraints. The first one is Kaluza 
