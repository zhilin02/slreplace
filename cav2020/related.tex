...
the question of solving satisfiability of \emph{word equations} (i.e., strings equation containing concatenation of string constants and variables). Makanin showed decidability \cite{Makanin}, but the upper bound was improved to PSPACE about 25 years later \cite{P04} and  %using a word compression technique. 
a simpler algorithm was given recently \cite{J17}. %using the recompression technique. 
...
(e.g., finite-state transductions), which motivates research on more expressive string constraint languages.   %, and so a path feasibility solver nonetheless needs to be able to
%handle them. 
...
A recent research endeavor %years have seen the possibility of recovering 
