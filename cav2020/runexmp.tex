...
%\end{example}
%
Note that {\urlxsssanitise} uses JavaScript the sanitisation operation $\sf trim$ that removes whitespace from both ends of a string, which can be conveniently modelled by finite-state transducers, and the two string functions involving integer data type, namely, $\sf indexof$ and $\sf substr$ (called $\indexof$ and $\substring$ in this paper), as well as concatenation and $\sf replace$ (with the g, global, flag, called $\replaceall$ in this paper). 
