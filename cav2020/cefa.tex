...
is a cost register update function. \zhilin{I restore to integers, since an integer cost register can be encoded by two monotonic registers.}\\
...
For convenience, we use $\CEFA_{\indexof_v}$ which accepts $\{(w, (n, \indexof_v(w, n)))\mid w\in \Sigma^* \}$. %to denote the constructed CEFA $\CEFA'$ for the special NFA $\NFA$ such that $\Lang(\NFA)=\Sigma^*$. 
...
\tl{do I need to worry about that the return value could be -1, if $a$ does not appear at all?}\zhilin{this corner case should be dealt by the decision procedure, instead of the CEFA, but we had better add a sentence to clarify this.}
\end{example}
