...
We have implemented the decision procedure on top of the string solver OSTRICH \cite{CHL+19}, resulting into a new solver OSTRICH+. OSTRICH is based on the SMT solver Princess and .
The pseudo-code is in Algorithm~\ref{alg:dfs}.
...
\SetKw{Continue}{continue}
  \small
  \KwIn{$active, passive$: sets of CEFA constraints,  $arith$: set of arithmetic constraints,
%    $x \in L$,
    $funApps$: acyclic set of assignment statements. }
  ...
  \Begin{
    \eIf{$\mathit{active} = \emptyset$}{
      ...
      \Return{$\mathit{symbolicCheckSat}(passive, arith)$;} 
    }{
	...
	\hspace*{4mm} each partition $\mathcal{J}_1, \mathcal{J}_2, \mathcal{J}_3$ of the set of $y:=\substring(x, i, j)$ in $funApps$ 
	}
	{
		...
		\For{each $\length(x)$ occurring in $arith$}
		{
			...
			$active \leftarrow active \cup \{x \in \CEFA_{\rm len}[i/r_1]\}$\; 
			$arith\leftarrow arith[i/\length(x)]$;
		}
		...
		\eIf{there is an assignment~$x := f(y_1, \vec{i_1}, \ldots, y_l,\vec{i_l})$ defining $x$ in $funApps$ with \newline
...
%		      	\tcc{Compute the $R(\CEFA)$-cost enriched pre-image of $\Lang(\CEFA)$ under $f$}
			compute $f^{-1}_{R(\CEFA)}(\Lang(\CEFA)) = \left((\CEFA^{(1)}_{j}, \cdots, \CEFA^{(l)}_{j})_{j \in [n]}, \vec{t}\right)$ where \newline 
			 $R(\CEFA^{(j')}_{j})=\left((r')^{(j', 1)}, \cdots,(r')^{(j', k_{j'})}, r^{(j')}_1,\cdots, r^{(j')}_k \right)$ for $j \in [n]$ and $j' \in [l]$\;
		        ...
		        \For{$j \leftarrow 1$ \KwTo $n$}{
          			...
          			\eIf{$active \cup passive$ is inconsistent}{
            				\Return{unsat};
          			}
				{
			          \Switch{$\mathit{checkSat}(active, passive, arith, funApps)$}{
					\uCase{$sat$}
						{\Return{$sat$};}
				        \Case{$unsat$}
      	...
      	{
        	\Return{$\mathit{checkSat}(active \backslash \{x\in \CEFA\}, passive \cup \{x\in \CEFA\}, arith, funApps)$;}
      	}
	} 
}
}
  \caption{Recursive function~$\mathit{checkSat}$
    for Step II-V \label{alg:dfs} of the decision procedure}
\end{algorithm}
  ...
  \KwResult{ }
		...
		\For{each $\indexof_v(x, i) \in \mathcal{I}_5$}
		{
			...
			$arith \leftarrow arith[-1/\indexof_v(x,i)] \wedge i \ge 0 \wedge i < \length(x) \wedge j = \length(x)-i$\;
			 $funApps \leftarrow funApps \cup \{y:=\substring(x, i, j)\}$\;
		}		
...
\begin{algorithm}[htbp]
  ...
  \KwResult{ }
		...
		\For{each $y:=\substring(x, i, j) \in \mathcal{J}_2$}
		{
			 ...
			 $funApps \leftarrow funApps[y:=\substring(x, i, i')/y:=\substring(x, i, j)]$\;
		}
		\For{each $y:=\substring(x, i, j) \in \mathcal{J}_3$}
		{
			 ...
			 $funApps \leftarrow funApps \setminus \{y:=\substring(x, i, j)\}$\;		 
		}
