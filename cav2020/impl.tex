...
We have implemented the decision procedure based on the recent string solver OSTRICH \cite{CHL+19}, resulting into a new solver OSTRICH+ \tl{OSTRICH2?}. OSTRICH is  written in Scala and is based on the SMT solver Princess \cite{princess08}. 
...
However, our preliminary experiments show that this approach suffers from scalability issue, in particular, %faces 
the state explosion when computing products of CEFAs.  %We did implement this approach in the beginning. However, some preliminary experiments showed that this approach is not scalable. Therefore, 
