...
For each integer term of the form $\indexof_v(x,i)$ in $S$, nondeterministically choose one of the following options (which correspond to the semantics of $\indexof_v$ in Section~\ref{sec:logic}).
\begin{itemize}
\item Add $\ASSERT{i < 0}$ to $S$, and replace $\indexof_v(x,i)$ with $\indexof_v(x,0)$ in $S$. 
%
\item Add $\ASSERT{i < 0};\ASSERT{x \in \NFA_{\overline{\Sigma^*v\Sigma^*}}}$ to $S$, and replace $\indexof_v(x,i)$ with $-1$ in $S$.
%
\item Add $\ASSERT{i \ge \length(x)}$ to $S$, and replace $\indexof_v(x,i)$ with $-1$ in $S$.
%
...
%
\item Add the statement $\ASSERT{i < 0}; \ASSERT{y \in \NFA_\varepsilon}$ to $S$, and  remove $y:=\substring(x, i, j)$ from $S$, where $\NFA_\varepsilon$ is the NFA defining the language $\{\varepsilon\}$.
\end{itemize}
