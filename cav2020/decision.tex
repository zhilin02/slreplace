...
pre-images of cost-enriched regular languages under the string operations in {\slint} (i.e., concatenation $\concat$, $\replaceall_{e,u}$, $\reverse$, FFTs $\NFT$, and $\substring$) are representable by so called recognizable relations (Section \ref{sect:pre}). \tl{will improve writing later}
...
Nondeterministically choose a subset $\mathcal{I}$ of the integer terms of the form $\indexof_v(x,i)$ in $S$, and for each $\indexof_v(x,i) \in \mathcal{I}$, do the following.
\begin{quote}
Replace $\indexof_v(x, i)$ with $-1$, and add the statements $y:=\substring(x, i, \length(x) - i); \ASSERT{x \in \NFA_{\overline{\Sigma^*v\Sigma^*}}}$ to $S$, where $y$ is a fresh variable and $\NFA_{\overline{\Sigma^*v\Sigma^*}}$ is an NFA defining the language $\{w \in \Sigma^*\mid v \mbox{ does not occur as a substring in } w\}$. 
\end{quote}
...
%
\item Add the statements $\ASSERT{i \ge 0}; \ASSERT{i+j  > \length(x)}$ to $S$, moreover, replace $y:=\substring(x, i, j)$ with $y:=\substring(x, i, \length(x) - i)$.
%
\item Add the statement $\ASSERT{i < 0}$ to $S$, moreover, replace $y:=\substring(x, i, j)$ with $y := \varepsilon$.
\end{itemize}
...
For each term $\indexof_v(x, i)$ occurring in $S$, introduce two fresh integer variables $i_1$ and $i_2$, replace every occurrence of $\indexof_v(x, i)$ by $i_2$, and add the statements $\ASSERT{i=i_1}; \ASSERT{x \in \CEFA_{\indexof_v}[i_1/r_1, i_2/r_2]}$ to $S$.  (See Example~\ref{exm:indexof} for an illustration of $\CEFA_{\rm \indexof_v}$.)\zhilin{The full construction is in the appendix}
...
\medskip
\noindent {\bf Step IV: Removing the assignment statements}.
...
Moreover, let $\phi$ denote the conjunction of all the LIA formulas occurring in $S$. It is straightforward to observe that $\phi$ is over %the set of integer variables 
$R'=\bigcup_{x\in X, j \in [s_x]}R(\CEFA_{x}^{j})$. Then the path feasibility of $S$ is reduced to deciding 
\begin{quote}
whether $\phi$ is satisfiable w.r.t. $(\Lambda_x)_{x \in X}$, namely,  %\begin{definition}[{\lasat} Problem]\label{def-la-sat-cefa}
