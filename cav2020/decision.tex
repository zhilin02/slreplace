...
we \emph{nondeterministically guess on the fly a disjunct of $\varphi_x$ out of the CEFAs in $\Lambda_x$ in polynomial space}, %which is doable according to 
...
An example to demonstrate the decision procedure applied to the program associated with {\tt urlXssSanitise(url)} in Section~\ref{sec:intro} is provided in the Appendix, Section~\ref{app:urlexample}.
