...
pre-images of cost-enriched regular languages under the string operations in {\slint} (i.e., concatenation $\concat$, $\replaceall_{e,u}$, $\reverse$, FFTs $\NFT$, and $\substring$) are representable by so called recognisable relations (Section \ref{sect:pre}). The details of the decision procedure are in Section~\ref{sec:dc}. \tl{will improve writing later}
