...
%
\begin{quote}
Suppose $y := f(x_1, \vec{i_1}, \cdots, x_l, \vec{i_l})$ is the last assignment of $S$, where $f: (\Sigma^* \times \Int^{k_1}) \times \cdots \times (\Sigma^* \times \Int^{k_l}) \rightarrow 2^{\Sigma^*}$ is a string function. 
...
\medskip
\noindent{\bf Step III: Final satisfiability checking.}%Solving the {\lasat} problem}.
...
For each $x \in X$, let $\CEFA_x$ denote the product of all CEFAs $\CEFA$ such that $\ASSERT{x \in \CEFA}$ appears in $S$ where the vector of the cost registers in $\CEFA_x$ is denoted by $R_x$. Let $\phi$ denote the conjunction of all the LIA formulas occurring in $S$. It is straightforward to observe that $\phi$ is over %the set of integer variables 
$\bigcup_{x\in X}R_x$. Then the path feasibility of $S$ is reduced to deciding 
\begin{quote}
whether $\phi$ is satisfiable w.r.t. $(\CEFA_x)_{x \in X}$, namely,  %\begin{definition}[{\lasat} Problem]\label{def-la-sat-cefa}
...
\begin{proposition}
	Given a family of CEFAs $\{ \CEFA_i^{j} \}_{i\in I,j\in J_i}$ each of which carries a vector of registers $R_i^j$ and a quantifier-free LIA formula $\phi$ such that  $ R_i^{j} $ are pairwise disjoint and the variables of $\phi$ are contained by $\bigcup_{i,j} R_i^j$. Deciding whether  %such that the free variables of $\phi$ are from $\bigcup_{i\in [m]} R_i$,  and here are 
	there are an assignment function $\theta: \bigcup_{i,j}R_i^j \rightarrow \Int$ and strings $(w_i)_{i \in I}$ such that  $\phi[(\theta(\bigcup_{i,j}R_i^j )/\bigcup_{i,j}R_i^j )]$ holds and $(w_i, \theta(\bigcup_{j\in J_i} R_i^j)) \in \bigcap_{j\in J_i} \Lang(\CEFA_i)$ for every $i \in I$ is PSPACE-complete. 
\end{proposition}
...
\paragraph*{Complexity analysis.} Step I can be done in linear time. %The complexity analysis of Step II is similar to that in \cite{CHL+19}. Roughly speaking, during 
In Step II, for each input string variable $x$ in $S$, at most exponentially many CEFAs can be generated for $x$, each of which is of at most exponential size. Therefore, Step II can be done in nondeterministic exponential time. For Step III, for each input string variable $x$, we compute the product $\CEFA_x$ of all CEFAs for $x$, which has at most doubly exponential number of states and singly exponential number of registers. By the same argument of Proposition 16 and Lemma 13 \cite{LB16}, we can generate an LIA formula of exponential size, and its satisfiability can be checked in EXPSPACE. \tl{to be checked later}
