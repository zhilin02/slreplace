...
We let $u[i]$ denote the $i$-th letter of $u$ (i.e., $a_i$) and $u[i,j]$ for the substring of $u$ starting from $i$ and ending with $j$ (i.e., $a_i\cdots a_j$). \tl{check later for consistency}\zhilin{the indices start from 0, to be consistent with the semantics of substring and indexof}\zhilin{i do use it in section 4}
...
If $u=a_1 \cdots a_n$ is nonempty, then we use $u^R$ to denote the \emph{reverse} of $u$, that is, $u^R= a_n \cdots a_1$. \tl{$u^r$ check later for consistency}
...
\paragraph*{Linear integer arithmetic.}  A linear integer arithmetic (abbreviated as LIA) formula $\phi$ is defined by the following rules
\[
\begin{array}{l c l}
...
\end{array}
\]
The \emph{size} of a LIA formula $\phi$, denoted by $|\phi|$, is defined as the number of symbols in $\phi$.
Let $\phi$ be a LIA formula and $i$ be a variable occurring in $\phi$. Then an occurrence of $i$ in $\phi$ is said to be \emph{free}  if the occurrence is not under the scope of $\exists i$. A formula $\phi$ is \emph{quantifier-free} if it does not contain quantifiers. The semantics of LIA formulas is standard and its definition is omitted here.
For a quantifier-free formula LIA formula $\phi$ that contains the free variables $i_1, \cdots, i_k$, we use $\cM(\phi)$ to denote the set of models of $\phi$, namely, $\cM(\phi) = \left\{(n_1, \cdots , n_k) \in \Int^k \mid \phi[n_1/i_1, \cdots, n_k/i_k] \mbox{ is evaluated to } true\right\}$, where $\phi[n_1/i_1, \cdots, n_k/i_k]$ is the formula obtained from $\phi$ by simultaneously replacing $i_1,\cdots, i_k$ with $n_1,\cdots, n_k$. An \emph{existential} LIA formula is a LIA formula where all the existential quantifiers are under the scope of an even number of negation symbols.
