...
We write $\nat$ and $\Int$ for the set of natural and integer numbers respectively. For $n \in \nat$ with $n \ge 1$, we use $[n]$ to denote $\{0, \cdots, n-1\}$. \zhilin{changed the definition of [n]} Throughout the paper, we use $\Sigma$ to denote a finite alphabet which is usually ranged over by $a,b,\cdots$.
...
Assume $u=a_0\cdots a_{n-1}$ is nonempty and $i<j \in [n]$. %Then a \emph{position} of $u$ is a number $i \in [|u|]$ (Note that the first position is $1$, instead of  0). In addition, 
We let $u[i]$ denote the $i$-th letter of $u$ (i.e., $a_i$) and $u[i,j]$ for the substring of $u$ starting from $i$ and ending with $j$ (i.e., $a_i\cdots a_j$). \tl{check later for consistency}\zhilin{the indices start from 0, to be consistent with the semantics of substring and indexof}
