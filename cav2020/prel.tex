...
We let $u[i]$ denote the $i$-th letter of $u$ (i.e., $a_i$) and $u[i,j]$ for the substring of $u$ starting from $i$ and ending with $j$ (i.e., $a_i\cdots a_j$). \tl{check later for consistency}\zhilin{the indices start from 0, to be consistent with the semantics of substring and indexof}\zhilin{confirm later on that we need the notation $u[i,j]$, if not, remove it}
