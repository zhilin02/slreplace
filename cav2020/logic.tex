...
%
\item The assignment $x:=\substring(y, t_1, t_2)$ denotes that $x$ is the substring of $y$ from the position $t_1$ to the position $t_2-1$. \zhilin{to be refined}
%
...
%
\item The integer term $\indexof_v(x, i)$ denotes the starting position of the first occurrence of $v$ in $x$ after the position $i$.  \zhilin{to be refined}
\end{itemize}
We would like to remark that the function ${\sf charAt}(x, i)$ can be seen as a special case of $\substring$, namely ${\sf charAt}(x, i) \equiv \substring(x, i, i+1)$.
