...
\[
\begin{array}{l}
y := \substring(x, 0, \indexof(x, c)); \ASSERT{y \in \NFA_{a^*}}; \\
z:= \substring(x, \indexof(x, c)+1,\length(x) - \indexof(x,c)-1); \\
\ASSERT{z \in \NFA_{b^*}}; \ASSERT{\length(y) = \length(z)}
\end{array}
\] 
is in {\slint}, where $\NFA_{a^*}$ and  $\NFA_{b^*}$ are the NFAs for the regular expressions $a^*$ and $b^*$ respectively. Intuitively the program specifies that $x \in \Lang(a^* c b^*)$, and $y,z$ are the substrings of $x$ before resp. after the occurrence of $c$, moreover, the lengths of $y$ and $z$ are the same.
\end{example}
...
\begin{remark}
The function $\replaceall_{e,u}$ can be seen as a special case of FFT, although the transformation into FFT may incur an exponential blowup \cite{CCH+18}.
\end{remark}
...
\end{array}
\] 
where $z_1,z_2$ are two freshly introduced string variables, and $\NFA_a$ (resp. $\NFA_{\Sigma \setminus a}$) is the NFA defining $\{a\}$ (resp. $\Sigma \setminus \{a\}$). Intuitively, two strings are different if their lengths are different or there exists some position where the characters of the two strings are different.
\end{remark}
