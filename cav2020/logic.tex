...
%\end{array}
%\]
We require that the string-manipulating programs are in {\bf single static assignment (SSA) form}. Note that the SSA form puts restrictions only on the assignment statements, but does not restrict the conditional statements.
...
\end{itemize}
We would like to remark that the function ${\sf charAt}(x, i)$ can be seen as a special case of $\substring$, namely ${\sf charAt}(x, i) \equiv \substring(x, i, 1)$.
