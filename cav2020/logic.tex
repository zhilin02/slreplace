...
\end{array}
\] 
is in {\slint}, where $\Sigma \setminus a$ (where $a \in \{(,)\}$) is the regular expression $(+b)_{b \in \Sigma \setminus \{a\}}$. Intuitively the program states that $x$ contains the same number of occurrences of ``('' and ``)'', moreover, the first occurrence of ``('' is before the first occurrence of ``)''.
\end{example}
