...
\begin{array}{l}
   ...
   \mathtt{querfrag1} := \substring(\mathtt{url1, qmarkpos}, \length(\mathtt{url1}) - \mathtt{qmarkpos});\\
    ...
    \ASSERT{\mathtt{querfrag2} \in \NFA_{\mathtt{<script>}}}
\end{array}
\]
where $\NFA_\varepsilon$ is the NFA defining $\{\varepsilon\}$, $\NFT_{\rm trim}$ is an NFT to model the sanitisation operation {\tt trim()}, and $\NFA_{\mathtt{<script>}}$ is the NFA defining $\{\mbox{\tt <script>}\}$. 
%
%
