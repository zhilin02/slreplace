...
We consider symbolic execution of string-manipulating programs with numeric conditions (abbreviated as {\slint}), defined by the following rules,% satisfies the two semantic conditions,
%
\[
\begin{array}{l c l}
...
%\end{array}
%\]
We require that the string-manipulating programs are in the {\bf single static assignment (SSA) form}. Note that the SSA form puts restrictions only on the assignment statements, but does not restrict the conditional statements. 
\tl{what does conditional statement mean? you mean asserations?}
...
\[ \substring(y, t_1, t_2)=
\begin{cases}
\epsilon & if t_1<0\vee t_1>|y| \vee t_2=0 \\
y[t_1, t_1+t_2-1] & o/w
\end{cases}
\]
