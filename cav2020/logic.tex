...
The program $S$ corresponding to the ``if'' branch of {\tt urlXssSanitise(url)} in Example~\ref{exmp:running} is an {\slint} program, if its syntax is slightly adapted. In particular, {\tt url1=url.trim()} in $S$ is transformed into ${\tt url1}:=\NFT_{\rm trim}({\tt url})$, where $\NFT_{\rm trim}$ is an NFT to model the sanitisation operation {\tt trim()}, and 
{\tt
%\begin{minted}{javascript}
querfrag2 = querfrag1.replace(/<script>|javascript:/g, '');
%\end{minted}
}
is changed into the statement ${\tt querfrag2}:=\replaceall_{e,\varepsilon}({\tt querfrag1})$, where $e$ is the regular expression {\tt $<$script$>$+javascript:}.
%\[
%\begin{array}{l}
