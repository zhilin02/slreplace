...
Integer data-types occur naturally and extensively in string-manipulating programs. %For instance, access to lengths or positions of strings is frequently needed. 
%
...
restricting the form of constraints (e.g.,~\cite{CCH+18,CHL+19}; cf.\ related work for a brief survey), adding length constraints would immediately lead to undecidability~\cite{CCH+18}. It is still a major open problem whether the string theory with concatenation (arguably the simplest string operation) and length function (arguably the most common string-number conversion) is decidable~\cite{Vijay-length}. 
%
...
reasoning about the combination strings and integers. Such solvers
...
constraints can cause solvers to diverge and not terminate. There is
also still a mismatch between the string operations supported by
...
by many solvers. 
The \emph{grand challenge} in string solving is to
