...
%Their generic nature can, however, lead to 
...
\cite{owasp17}. Integer  %data type 
comes naturally with string-manipulating programs. %For instance, access to lengths or positions of strings is frequently needed. 
...
functions such as $\length$, $\indexof$, and $\substring$ which can convert strings to integers and vice verse. %, are also widely used in string-manipulating programs. 
...
(also called symbolic executions) through the program under consideration. %software system under test.
...
string-number conversion) would render the theory undecidable \cite{buchi,GB16}. A further evidence is that, although a great deal of research has shown that one can support reasoning about rather complex string operations without breaking decidability by imposing syntactic restrictions on the constraints (e.g., \cite{CCH+18,CHL+19}; cf. related work for a brief survey), adding length constraints would immediately lead to undecidability \cite{CCH+18}. At the same time, the classes of constraints that are required in practice sometimes require solving techniques which are not necessarily complete. %string operations that are not covered by decidable string constraint languages.
%
