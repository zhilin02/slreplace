...
Of course, RWREs themselves are commonly in practice used in combination with
other string operations, e.g. match and replace(all) functions \cite{LMK19}. Their semantics drastically affect the behaviors of these functions. 
...
in $e$. 
To find the matches of RWREs in strings, one must take a special care of the
...
RWREs have also received the attentions in software engineering community. Some empirical studies have been reported for RWREs, including portability across different programing languages \cite{DMC+19} and DDos attacks \cite{SP18}, as well as how programmers write RWREs in practice \cite{MDD+19}.
