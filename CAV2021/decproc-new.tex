...
With Lemma~\ref{lem-extract}-\ref{lem-replace}, the path feasibility of $\strline_{\sf reg}$ is reduced to the path feasibility of string-manipulating programs that are a sequential composition of the statements of the form $z:=x \concat y$, $y:=\cT(x)$ and $\ASSERT{x \in \cA}$, where $\cT$ is a PSST and $\cA$ is an FA. Let us use  $\strline'_{\sf reg}$ to denote the class of such programs. We will  follow the backward reasoning approach of the OSTRICH solver proposed in \cite{CCH+18,CHL+19} to solve the path feasibility of $\strline'_{\sf reg}$. The key is to show that the pre-images of regular languages  under PSSTs  are regular and can be computed effectively.
...
With Lemma~\ref{lem:psst_preimage}, we then solve the path feasibility of $\strline'_{\sf reg}$ by repeating the following procedure, until no more assignment statements are left:\\
...
Intuitively, $\cT_{\extract_{i,e}}$ simulates $\cA_e$ and stores the letters in a string variable $x$, when applying the transitions in ${\sf Sub}_{e'}[\cA_e]$.

Suppose $\cA_e = (Q, \Sigma, \delta, \tau, q_0, f_0)$. 
Then $\cT_{\extract_{i,e}} = (Q  \cup \{q'_0\}, \Sigma, X, \delta', \tau', E', q'_0, F')$ (see Figure~\ref{fig-psst-extract}), where 
\begin{itemize}
\item $q'_0 \not \in Q$,
...
%
\item $\tau'(q'_0) = ((q_0), ())$, moreover, if $q_0$ occurs in ${\sf Sub}_{e'}[\cA_e]$, then $E'((q'_0,\varepsilon, q_0)) = \varepsilon$, otherwise, $E'((q'_0,\varepsilon, q_0)) = {\sf null}$, 
%
\item all the transitions of $\delta$ (resp. $\tau$) are in $\delta'$ (resp. $\tau'$), and the values of $E'$ on these transitions are defined as follows,
%
\begin{itemize}
\item for every transition $(p, \sigma, q)$ with $\sigma \in \Sigma$ in ${\sf Sub}_{e'}[\cA_e]$, $E'((p, \sigma, q))(x) = x \sigma$,
%
\item for every transition $(p, \varepsilon, q)$ in $\cA_e$ such that $q$ is the initial state of ${\sf Sub}_{e'}[\cA_e]$ and $p$ does not occur in ${\sf Sub}_{e'}[\cA_e]$, $E'((p, \varepsilon, q))(x) = \varepsilon$,
% 
\item for all the other transitions $(p, \sigma, q)$ with $\sigma \in \Sigma$ (resp. $(p, \varepsilon, q)$)  in $\cA_e$, $E'((p, \sigma, q))(x) = x$ (resp. $E'((p, \varepsilon, q))(x) = x$),
\end{itemize}
%
\item $F'(f_0) = x$.
\end{itemize}
...
\centering
%\rule{\linewidth}{0cm}
\includegraphics{psst-extract.pdf}
...
Let $\cT$ be the PSST $\cT_{\tt match_{decimalReg,1}}$ in Figure~\ref{fig-psst-exmp} and $\cA$ be the FA corresponding to the regular expression $\{1,\cdots,9\}^*$, specifically, $\cA= (\{p_0\}$, $\{0,\cdots,9\}$, $\delta_A$, $p_0, \{p_0\})$, where $\delta_A = \{(q_0, \ell, q_0) \mid \ell = 1, \cdots, 9\}$.
