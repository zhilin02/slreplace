...
Intuitively, $\cT_{\extract_{i,e}}$ uses a string variable $x$ to store the value of the $i$th capturing group, initially it assigns $\nullchar$ to $x$, then simulates $\cA_e$, and stores letters into $x$ when applying the transitions in ${\sf Sub}_{e'}[\cA_e]$, finally it outputs the value of $x$ when $\cA_e$ accepts.
...
\begin{itemize}
\item if $\tau'(q'_0) = ((q_0); ())$ and $E'((q'_0, \varepsilon, q_0))(x) = \nullchar$, 
%
\item $\delta'$ includes all the transitions in $\delta$ and $\tau'$ includes all the transitions in $\tau$,
%
\item for each transition $(q, \sigma, q')$ in ${\sf Sub}_{e'}[\cA_e]$, $E'((q, \sigma, q'))(x) = x \sigma$, and for all the other transitions $t$ of $\cA_e$, $E'(t)(x) = x$.
%
%
\end{itemize}
\end{itemize}
