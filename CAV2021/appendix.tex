...
\noindent{\bf Lemma~\ref{lem-extract}}. 
\emph{A PSST $\cT_{\extract_{i,e}}$ can be constructed for $\extract_{i,e}$ such that 
$$\cR_{\cT_{\extract_{i,e}}} = \{(w, w') \mid w'= \extract_{i,e}(w)\}.$$}
...
\begin{proof}	%\paragraph*{Construction of $\cT_{\replaceall_{\pat, \rep}}$.} 
	...
	Suppose $\cA_\pat = (Q, \Sigma, \delta, \tau, q_0, f_0)$. Then $\cT_{\replace_{\pat,\rep}}$ is obtained from $\cA_\pat$ by adding a fresh state $q'_0$ such that (see Fig.~\ref{fig-psst-replace})
	\begin{itemize}
		...
		%
		\item when $\cT_{\replace_{\pat,\rep}}$ stays at $q'_0$, it keeps appending the current letter to the end of $x_0$, which stores the output of $\cT_{\replace_{\pat,\rep}}$,
		%
		...
		%
		\item when the first match of $\pat$ is found, $\cT_{\replace_{\pat,\rep}}$ goes from $f_0$ to $q'_0$ via an $\varepsilon$-transition, appends the replacement string, which is $\rep[x_1/\$_{i_1}, \cdots, x_k/\$_{i_k}]$, to the end of $x_0$, resets the values of $x_1,\cdots,x_k$ to $\varepsilon$ and keeps searching for the next match of $\pat$.
	\end{itemize}
	
	\begin{figure}[ht]
		\centering
		%\rule{\linewidth}{0cm}
		\includegraphics[scale=0.9]{psst-replace.pdf}
