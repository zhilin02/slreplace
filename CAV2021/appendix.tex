...
%\begin{proof}	%\paragraph*{Construction of $\cT_{\replaceall_{\pat, \rep}}$.} 
	Next, we show how to construct the PSST for $\replaceall_{\pat, \rep}$. The construction of the PSST for $\replace_{\pat, \rep}$ is similar and simpler, thus omitted.
	...
	Suppose $\cA_\pat = (Q, \Sigma, \delta, \tau, q_0, f_0)$. Then $\cT_{\replaceall_{\pat,\rep}}$ is obtained from $\cA_\pat$ by adding a fresh state $q'_0$ such that (see Fig.~\ref{fig-psst-replace})
	\begin{itemize}
