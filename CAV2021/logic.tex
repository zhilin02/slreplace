...
\section{The String-Manipulating Language}\label{sec:logic}
...
%
	\item  for the $\replace$ or $\replaceall$ operation, $\pat\in \cgexp$, $\rep \in \refexp$, %$\replaceall$ is the replace-all function to be defined shortly,
...
matches of $\pat$ in $x$ and replace them with the corresponding strings specified by the replacement string $\rep$. (In $\rep$,  references may be used to refer to %are replaced by 
...
We shall show that the path feasibility problem is decidable, if the uses of back references in assertion statements are forbidden, which turns out to be the situation in practice.\footnote{An evidence was given in  \cite{LMK19}, where the statistics showed that the occurrences of regular expressions with back references occupy only less than $1\%$ in the widely used JavaScript NPM packages.} In the sequel, we will use $\strline_{\sf reg}$ to denote the collection of $\strline$ programs which are free of %where no 
