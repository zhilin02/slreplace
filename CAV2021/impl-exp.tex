...
\begin{table}[tb]
	...
	\hline
	   & &
	  \multicolumn{6}{c|@{\quad}}{\textbf{Aratha+\ostrich}} &\multicolumn{6}{c@{\quad}|}{\textbf{\expose{}+Z3}}
	  \\
    & &
	  \multicolumn{6}{c|@{\quad}}{paths covered within 60s} &\multicolumn{6}{c@{\quad}|}{paths covered within 60s}
	  \\
	   & \#JS-Progs. & ~0~  & ~1~ &  ~2~ & ~3~ & ~$\geq$4~ & ~\#T.O.~ &
	    ~0~  & ~1~ &  ~2~ & ~3~ & ~$\geq$4~ & ~\#T.O.~
	  \\\hline
	  \textbf{\expose{}} & 51 & 14 & 9 & 11 & 2 & 15 & 0  & 16 & 7 & 11 & 2 & 15 & 1  
	  \\
	  && \multicolumn{6}{c|@{\quad}}{Average time: \textbf{8.57}s} & \multicolumn{6}{c@{\quad}|}{Average time: 14.57s}
	  \\\hline
	  \textbf{Match} & 28 & 3 & 7 & 12 & 6 & 0 & 0  & 3 & 8 & 12 & 5 & 0 & 6
	  \\
	  && \multicolumn{6}{c|@{\quad}}{Average time: \textbf{4.30}s} & \multicolumn{6}{c@{\quad}|}{Average time: 23.66s}
	  \\\hline
	  \textbf{Replace} & 38 & 12 & 20 & 6 & 0 & 0 & 0  & 12 & 23 & 3 & 0 & 0 & 22
	  \\
	  && \multicolumn{6}{c|@{\quad}}{Average time: \textbf{2.71}s} & \multicolumn{6}{c@{\quad}|}{Average time: 41.73s}
	  \\\hline
	\end{tabular}
	\end{center}
...
is faster than \expose{}+Z3 by a factor between 2 and 15, while being
able to cover more paths. The smaller speedup compared to the results
for \textbf{R2} can be explained by the fact that Aratha should spend time in processing Javascript code, besides using {\ostrich} to solve path queries.\zhilin{I adapted this sentence since Denghang only did the experiments on those string-manipulating programs. @Philpp, please check.}
%SMT-LIB queries
