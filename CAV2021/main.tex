...
Regular expressions (RE) are a classical concept in formal language theory, which are expressions built from characters by the operators of concatenation, union, and Kleene star. Real-world regular expressions (RWRE) in programming languages differ from REs in the non-standard semantics of operators, e.g. non-commutative union and greedy/lazy Kleene star, as well as  the additional features like capturing groups and back references. 
...
While REs are supported by every state-of-the-art string constraint solvers (e.g. CVC4 and Z3-str), RWREs are not supported by them. The mismatch between REs in string constraint solvers and RWREs in programming languages hinders the precision and efficiency of symbolic executors, which have to resort to approximate encoding of RWREs and counter example guided abstraction refinements (CEGAR) to mitigate this issue. 
%
