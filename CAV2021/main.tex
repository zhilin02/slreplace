...
State-of-the-art string constraint solvers (e.g. CVC4 and Z3-str) do not support RWREs. Symbolic executors of string-manipulating programs have to resort to counter example guided refinements (CEGAR) to mitigate the imprecise semantics of REs in the solvers.  
%
...
%% contribution
In this work, we propose an approach of natively supporting RWREs in string constraint solving. We introduce a new automata model called prioritized streaming string transducers (PSST), which combines priorities in prioritized finite transducers and string variables in streaming string transducers, to model the string functions involving RWREs. 
