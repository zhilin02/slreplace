...
%Nevertheless, 
Note that the greedy semantics is crucial here, because with standard \emph{nondeterministic} semantics the {\tt ({\footnotesize\textbackslash}d+)} could also (incorrectly) by matched by \texttt{"02"}, and {\tt ({\footnotesize\textbackslash}d*)} by \texttt{"50"}.
...
A natural post-condition of {\tt normalize} is that the result contains neither leading nor trailing zeros. The post-condition holds if it is
established by the function on \emph{every} execution path. 
...
The post-condition entails that the following program in single static assignment form (cf. Fig.~\ref{fig-run-exmp-path}) is infeasible (namely, there does not exist an input of {\tt decimal} so that the program can execute to the end), 
\begin{figure}[tb]
\begin{center}
\begin{minted}[linenos]{javascript}
  const decimalReg = /^(\d+)\.?(\d*)$/;
