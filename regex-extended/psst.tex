...
In this section, we introduce Prioritized Finite-state automata (PFA) and Prioritized Streaming String Transducers (PSST), based on which we model the semantics of $\regexp$ defined in Section~\ref{sec:prel} and design the decision procedure in Section~\ref{sec:decision}.
...
We introduce  a new class of transducers which combine prioritized transducers \cite{BM17} %which combines the expressive power of 
