(declare-fun Constructed_Argument_t () String)
(declare-fun CreateAsClass () Bool)
(declare-fun Constructed_Argument_elements_routes_0_t () String)
(declare-fun Constructed_Argument_elements_mode_1_t () String)
(declare-fun Constructed_Argument_elements_base_2_t () String)
(declare-fun Constructed_Argument_elements_base_2 () String)
(declare-fun |0 Fill 0| () String)
(declare-fun |0 Fill 2| () String)
(declare-fun |0 Fill 1| () String)
(declare-fun |IsMatch_/\\/$/_0| () Bool)
(define-funs-rec ( ( str.repeat ((x!1 String) (x!2 Int)) String)
                   ( str.whiteLeft ((x!1 String) (x!2 Int)) Int)
                   ( str.whiteRight ((x!1 String) (x!2 Int)) Int))
                 ( (ite (<= x!2 0)
                        ""
                        (str.++ x!1 ((_ str.repeat 0) x!1 (- x!2 1))))
                   (ite (= (str.at x!1 x!2) " ")
                        ((_ str.whiteLeft 0) x!1 (+ x!2 1))
                        x!2)
                   (ite (= (str.at x!1 x!2) " ")
                        ((_ str.whiteRight 0) x!1 (- x!2 1))
                        x!2)))
(assert (= Constructed_Argument_t "object"))
(assert (not CreateAsClass))
(assert (not (= Constructed_Argument_elements_routes_0_t "string")))
(assert (not (= Constructed_Argument_elements_routes_0_t "number")))
(assert (not (= Constructed_Argument_elements_routes_0_t "boolean")))
(assert (not (= Constructed_Argument_elements_routes_0_t "object")))
(assert (not (= Constructed_Argument_elements_routes_0_t "array_number")))
(assert (not (= Constructed_Argument_elements_routes_0_t "array_string")))
(assert (not (= Constructed_Argument_elements_routes_0_t "array_bool")))
(assert (not (= Constructed_Argument_elements_routes_0_t "null")))
(assert (not (= Constructed_Argument_elements_mode_1_t "string")))
(assert (not (= Constructed_Argument_elements_mode_1_t "number")))
(assert (not (= Constructed_Argument_elements_mode_1_t "boolean")))
(assert (not (= Constructed_Argument_elements_mode_1_t "object")))
(assert (not (= Constructed_Argument_elements_mode_1_t "array_number")))
(assert (not (= Constructed_Argument_elements_mode_1_t "array_string")))
(assert (not (= Constructed_Argument_elements_mode_1_t "array_bool")))
(assert (not (= Constructed_Argument_elements_mode_1_t "null")))
(assert (= Constructed_Argument_elements_base_2_t "string"))
(assert (not (= Constructed_Argument_elements_base_2 "")))
(assert (not (= (str.at Constructed_Argument_elements_base_2 0) "/")))
(assert (= |0 Fill 0| "/"))
(assert (= |0 Fill 2| "/"))
(assert (let ((a!1 (str.in.re (str.++ "/" Constructed_Argument_elements_base_2)
                      (re.++ (re.* (re.range "\x00" "\xff")) (str.to.re "/")))))
  (or (not a!1)
      (= (str.++ "/" Constructed_Argument_elements_base_2)
         (str.++ |0 Fill 1| "/")))))
(assert (let ((a!1 (str.in.re (str.++ "/" Constructed_Argument_elements_base_2)
                      (re.++ (re.* (re.range "\x00" "\xff")) (str.to.re "/")))))
  (= a!1 |IsMatch_/\\/$/_0|)))
(assert |IsMatch_/\\/$/_0|)

(check-sat)
