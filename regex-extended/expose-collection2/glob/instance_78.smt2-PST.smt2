...
(assert (str.in.re (elimWhiteRight (elimWhiteLeft Constructed_Argument)) (re.++ (re.* (re.range "\x00" "\xff")) (str.to.re "{") (re.* (re.union (re.range "\x00" "\x09") (re.range "\v" "\xff"))) (str.to.re "}") (re.* (re.range "\x00" "\xff")))))
(check-sat)
