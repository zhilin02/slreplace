...
(assert (str.in.re (ite (<= (str.len (elimWhiteRight (elimWhiteLeft Constructed_Argument))) 0) "" (elimWhiteRight (elimWhiteLeft Constructed_Argument))) (re.++ (re.* (re.range "\x00" "\xff")) (str.to.re "/") (re.* (str.to.re "/")) (re.* (re.range "\x00" "\xff")))))
(check-sat)
