(declare-fun ExpandObjSwitcher () Real)
(declare-fun Constructed_Argument_t () String)
(declare-fun CreateAsClass () Bool)
(declare-fun |0 Fill 0| () String)
(declare-fun |0 Fill 1| () String)
(declare-fun |0 Fill 2| () String)
(declare-fun |0 Fill 3| () String)
(declare-fun |0 Fill 6| () String)
(declare-fun |0 Fill 5| () String)
(declare-fun |0 Fill 4| () String)
(declare-fun Constructed_Argument () String)
(declare-fun |IsMatch_/\\x1B\\[\\d+m/_0| () Bool)
(declare-fun |1 Fill 0| () String)
(declare-fun |1 Fill 1| () String)
(declare-fun |1 Fill 2| () String)
(declare-fun |1 Fill 3| () String)
(declare-fun |1 Fill 6| () String)
(declare-fun |1 Fill 5| () String)
(declare-fun |1 Fill 4| () String)
(declare-fun |IsMatch_/\\x1B\\[\\d+m/_1| () Bool)
(declare-fun |2 Fill 0| () String)
(declare-fun |2 Fill 1| () String)
(declare-fun |2 Fill 2| () String)
(declare-fun |2 Fill 3| () String)
(declare-fun |2 Fill 6| () String)
(declare-fun |2 Fill 5| () String)
(declare-fun |2 Fill 4| () String)
(declare-fun |IsMatch_/\\x1B\\[\\d+m/_2| () Bool)
(declare-fun |3 Fill 0| () String)
(declare-fun |3 Fill 1| () String)
(declare-fun |3 Fill 2| () String)
(declare-fun |3 Fill 3| () String)
(declare-fun |3 Fill 6| () String)
(declare-fun |3 Fill 5| () String)
(declare-fun |3 Fill 4| () String)
(declare-fun |IsMatch_/\\x1B\\[\\d+m/_3| () Bool)
(declare-fun |4 Fill 0| () String)
(declare-fun |4 Fill 1| () String)
(declare-fun |4 Fill 2| () String)
(declare-fun |4 Fill 3| () String)
(declare-fun |4 Fill 6| () String)
(declare-fun |4 Fill 5| () String)
(declare-fun |4 Fill 4| () String)
(declare-fun |IsMatch_/\\x1B\\[\\d+m/_4| () Bool)
(define-funs-rec ( ( str.repeat ((x!1 String) (x!2 Int)) String)
                   ( str.whiteLeft ((x!1 String) (x!2 Int)) Int)
                   ( str.whiteRight ((x!1 String) (x!2 Int)) Int))
                 ( (ite (<= x!2 0)
                        ""
                        (str.++ x!1 ((_ str.repeat 0) x!1 (- x!2 1))))
                   (ite (= (str.at x!1 x!2) " ")
                        ((_ str.whiteLeft 0) x!1 (+ x!2 1))
                        x!2)
                   (ite (= (str.at x!1 x!2) " ")
                        ((_ str.whiteRight 0) x!1 (- x!2 1))
                        x!2)))
(assert (= ExpandObjSwitcher 3.0))
(assert (= Constructed_Argument_t "string"))
(assert (not CreateAsClass))
(assert (= |0 Fill 0| "\x1B"))
(assert (= |0 Fill 1| "["))
(assert (str.in.re |0 Fill 2| (re.++ (re.range "0" "9") (re.* (re.range "0" "9")))))
(assert (= |0 Fill 3| "m"))
(assert (str.in.re |0 Fill 6|
           (re.++ (str.to.re "\x1B[")
                  (re.range "0" "9")
                  (re.* (re.range "0" "9"))
                  (str.to.re "m"))))
(assert (= |0 Fill 6| (str.++ "\x1B[" |0 Fill 2| "m")))
(assert (= Constructed_Argument (str.++ |0 Fill 4| "\x1B[" |0 Fill 2| "m" |0 Fill 5|)))
(assert (str.in.re Constructed_Argument
           (re.++ (re.* (re.range "\x00" "\xff"))
                  (str.to.re "\x1B[")
                  (re.range "0" "9")
                  (re.* (re.range "0" "9"))
                  (str.to.re "m")
                  (re.* (re.range "\x00" "\xff")))))
(assert |IsMatch_/\\x1B\\[\\d+m/_0|)
(assert (= |1 Fill 0| "\x1B"))
(assert (= |1 Fill 1| "["))
(assert (str.in.re |1 Fill 2| (re.++ (re.range "0" "9") (re.* (re.range "0" "9")))))
(assert (= |1 Fill 3| "m"))
(assert (str.in.re |1 Fill 6|
           (re.++ (str.to.re "\x1B[")
                  (re.range "0" "9")
                  (re.* (re.range "0" "9"))
                  (str.to.re "m"))))
(assert (= |1 Fill 6| (str.++ "\x1B[" |1 Fill 2| "m")))
(assert (let ((a!1 (>= (+ (str.len |0 Fill 4|) (* (- 1) (str.len Constructed_Argument)))
               0))
      (a!3 (ite (>= (+ (str.len |0 Fill 4|)
                       (str.len |0 Fill 6|)
                       (str.len Constructed_Argument))
                    0)
                (+ (str.len |0 Fill 4|)
                   (str.len |0 Fill 6|)
                   (str.len Constructed_Argument))
                0)))
(let ((a!2 (ite (<= (str.len Constructed_Argument) 0)
                ""
                (str.substr Constructed_Argument
                            0
                            (ite a!1
                                 (str.len Constructed_Argument)
                                 (str.len |0 Fill 4|)))))
      (a!4 (ite (>= (+ (str.len |0 Fill 4|) (str.len |0 Fill 6|)) 0)
                (+ (str.len |0 Fill 4|) (str.len |0 Fill 6|))
                a!3)))
(let ((a!5 (ite (<= (+ (str.len Constructed_Argument) (* (- 1) a!4)) 0)
                ""
                (str.substr Constructed_Argument
                            a!4
                            (+ (str.len Constructed_Argument) (* (- 1) a!4))))))
  (= (str.++ a!2 a!5) (str.++ |1 Fill 4| "\x1B[" |1 Fill 2| "m" |1 Fill 5|))))))
(assert (str.in.re (str.++ |1 Fill 4| "\x1B[" |1 Fill 2| "m" |1 Fill 5|)
           (re.++ (re.* (re.range "\x00" "\xff"))
                  (str.to.re "\x1B[")
                  (re.range "0" "9")
                  (re.* (re.range "0" "9"))
                  (str.to.re "m")
                  (re.* (re.range "\x00" "\xff")))))
(assert |IsMatch_/\\x1B\\[\\d+m/_1|)
(assert (= |2 Fill 0| "\x1B"))
(assert (= |2 Fill 1| "["))
(assert (str.in.re |2 Fill 2| (re.++ (re.range "0" "9") (re.* (re.range "0" "9")))))
(assert (= |2 Fill 3| "m"))
(assert (str.in.re |2 Fill 6|
           (re.++ (str.to.re "\x1B[")
                  (re.range "0" "9")
                  (re.* (re.range "0" "9"))
                  (str.to.re "m"))))
(assert (= |2 Fill 6| (str.++ "\x1B[" |2 Fill 2| "m")))
(assert (let ((a!1 (>= (+ (str.len |0 Fill 4|) (* (- 1) (str.len Constructed_Argument)))
               0))
      (a!3 (ite (>= (+ (str.len |0 Fill 4|)
                       (str.len |0 Fill 6|)
                       (str.len Constructed_Argument))
                    0)
                (+ (str.len |0 Fill 4|)
                   (str.len |0 Fill 6|)
                   (str.len Constructed_Argument))
                0)))
(let ((a!2 (str.len (str.substr Constructed_Argument
                                0
                                (ite a!1
                                     (str.len Constructed_Argument)
                                     (str.len |0 Fill 4|)))))
      (a!4 (ite (>= (+ (str.len |0 Fill 4|) (str.len |0 Fill 6|)) 0)
                (+ (str.len |0 Fill 4|) (str.len |0 Fill 6|))
                a!3)))
(let ((a!5 (str.len (str.substr Constructed_Argument
                                a!4
                                (+ (str.len Constructed_Argument) (* (- 1) a!4)))))
      (a!8 (* (- 1) (ite (<= (str.len Constructed_Argument) 0) 0 a!2))))
(let ((a!6 (ite (<= (+ (str.len Constructed_Argument) (* (- 1) a!4)) 0) 0 a!5)))
(let ((a!7 (+ (ite (<= (str.len Constructed_Argument) 0) 0 a!2) a!6))
      (a!10 (+ (str.len |1 Fill 4|)
               (str.len |1 Fill 6|)
               (ite (<= (str.len Constructed_Argument) 0) 0 a!2)
               a!6)))
(let ((a!9 (ite (>= (+ (str.len |1 Fill 4|) a!8 (* (- 1) a!6)) 0)
                a!7
                (str.len |1 Fill 4|)))
      (a!11 (ite (>= (+ (str.len |1 Fill 4|) (str.len |1 Fill 6|)) 0)
                 (+ (str.len |1 Fill 4|) (str.len |1 Fill 6|))
                 (ite (>= a!10 0) a!10 0))))
(let ((a!12 (+ (ite (<= (str.len Constructed_Argument) 0) 0 a!2)
               (* (- 1) a!11)
               a!6)))
(let ((a!13 (str.++ (ite (<= a!7 0)
                         ""
                         (str.substr (str.++ |1 Fill 4|
                                             "\x1B["
                                             |1 Fill 2|
                                             "m"
                                             |1 Fill 5|)
                                     0
                                     a!9))
                    (ite (<= a!12 0)
                         ""
                         (str.substr (str.++ |1 Fill 4|
                                             "\x1B["
                                             |1 Fill 2|
                                             "m"
                                             |1 Fill 5|)
                                     a!11
                                     a!12)))))
  (= a!13 (str.++ |2 Fill 4| "\x1B[" |2 Fill 2| "m" |2 Fill 5|)))))))))))
(assert |IsMatch_/\\x1B\\[\\d+m/_2|)
(assert (str.in.re (str.++ |2 Fill 4| "\x1B[" |2 Fill 2| "m" |2 Fill 5|)
           (re.++ (re.* (re.range "\x00" "\xff"))
                  (str.to.re "\x1B[")
                  (re.range "0" "9")
                  (re.* (re.range "0" "9"))
                  (str.to.re "m")
                  (re.* (re.range "\x00" "\xff")))))
(assert (= |3 Fill 0| "\x1B"))
(assert (= |3 Fill 1| "["))
(assert (str.in.re |3 Fill 2| (re.++ (re.range "0" "9") (re.* (re.range "0" "9")))))
(assert (= |3 Fill 3| "m"))
(assert (str.in.re |3 Fill 6|
           (re.++ (str.to.re "\x1B[")
                  (re.range "0" "9")
                  (re.* (re.range "0" "9"))
                  (str.to.re "m"))))
(assert (= |3 Fill 6| (str.++ "\x1B[" |3 Fill 2| "m")))
(assert (let ((a!1 (>= (+ (str.len |0 Fill 4|) (* (- 1) (str.len Constructed_Argument)))
               0))
      (a!3 (ite (>= (+ (str.len |0 Fill 4|)
                       (str.len |0 Fill 6|)
                       (str.len Constructed_Argument))
                    0)
                (+ (str.len |0 Fill 4|)
                   (str.len |0 Fill 6|)
                   (str.len Constructed_Argument))
                0)))
(let ((a!2 (str.len (str.substr Constructed_Argument
                                0
                                (ite a!1
                                     (str.len Constructed_Argument)
                                     (str.len |0 Fill 4|)))))
      (a!4 (ite (>= (+ (str.len |0 Fill 4|) (str.len |0 Fill 6|)) 0)
                (+ (str.len |0 Fill 4|) (str.len |0 Fill 6|))
                a!3)))
(let ((a!5 (str.len (str.substr Constructed_Argument
                                a!4
                                (+ (str.len Constructed_Argument) (* (- 1) a!4)))))
      (a!12 (* (- 1) (ite (<= (str.len Constructed_Argument) 0) 0 a!2))))
(let ((a!6 (ite (<= (+ (str.len Constructed_Argument) (* (- 1) a!4)) 0) 0 a!5)))
(let ((a!7 (+ (str.len |1 Fill 4|)
              (str.len |1 Fill 6|)
              (ite (<= (str.len Constructed_Argument) 0) 0 a!2)
              a!6))
      (a!11 (+ (ite (<= (str.len Constructed_Argument) 0) 0 a!2) a!6)))
(let ((a!8 (ite (>= (+ (str.len |1 Fill 4|) (str.len |1 Fill 6|)) 0)
                (+ (str.len |1 Fill 4|) (str.len |1 Fill 6|))
                (ite (>= a!7 0) a!7 0)))
      (a!13 (ite (>= (+ (str.len |1 Fill 4|) a!12 (* (- 1) a!6)) 0)
                 a!11
                 (str.len |1 Fill 4|))))
(let ((a!9 (+ (ite (<= (str.len Constructed_Argument) 0) 0 a!2)
              (* (- 1) a!8)
              a!6))
      (a!14 (ite (<= a!11 0)
                 0
                 (str.len (str.substr (str.++ |1 Fill 4|
                                              "\x1B["
                                              |1 Fill 2|
                                              "m"
                                              |1 Fill 5|)
                                      0
                                      a!13)))))
(let ((a!10 (ite (<= a!9 0)
                 0
                 (str.len (str.substr (str.++ |1 Fill 4|
                                              "\x1B["
                                              |1 Fill 2|
                                              "m"
                                              |1 Fill 5|)
                                      a!8
                                      a!9)))))
(let ((a!15 (ite (>= (+ (str.len |2 Fill 4|) (* (- 1) a!10) (* (- 1) a!14)) 0)
                 (+ a!10 a!14)
                 (str.len |2 Fill 4|)))
      (a!16 (ite (>= (+ (str.len |2 Fill 4|) (str.len |2 Fill 6|) a!10 a!14) 0)
                 (+ (str.len |2 Fill 4|) (str.len |2 Fill 6|) a!10 a!14)
                 0)))
(let ((a!17 (ite (>= (+ (str.len |2 Fill 4|) (str.len |2 Fill 6|)) 0)
                 (+ (str.len |2 Fill 4|) (str.len |2 Fill 6|))
                 a!16)))
(let ((a!18 (ite (<= (+ a!10 a!14 (* (- 1) a!17)) 0)
                 ""
                 (str.substr (str.++ |2 Fill 4|
                                     "\x1B["
                                     |2 Fill 2|
                                     "m"
                                     |2 Fill 5|)
                             a!17
                             (+ a!10 a!14 (* (- 1) a!17))))))
(let ((a!19 (str.++ (ite (<= (+ a!10 a!14) 0)
                         ""
                         (str.substr (str.++ |2 Fill 4|
                                             "\x1B["
                                             |2 Fill 2|
                                             "m"
                                             |2 Fill 5|)
                                     0
                                     a!15))
                    a!18)))
  (= a!19 (str.++ |3 Fill 4| "\x1B[" |3 Fill 2| "m" |3 Fill 5|)))))))))))))))
(assert |IsMatch_/\\x1B\\[\\d+m/_3|)
(assert (str.in.re (str.++ |3 Fill 4| "\x1B[" |3 Fill 2| "m" |3 Fill 5|)
           (re.++ (re.* (re.range "\x00" "\xff"))
                  (str.to.re "\x1B[")
                  (re.range "0" "9")
                  (re.* (re.range "0" "9"))
                  (str.to.re "m")
                  (re.* (re.range "\x00" "\xff")))))
(assert (= |4 Fill 0| "\x1B"))
(assert (= |4 Fill 1| "["))
(assert (str.in.re |4 Fill 2| (re.++ (re.range "0" "9") (re.* (re.range "0" "9")))))
(assert (= |4 Fill 3| "m"))
(assert (str.in.re (str.++ "\x1B[" |4 Fill 2| "m")
           (re.++ (str.to.re "\x1B[")
                  (re.range "0" "9")
                  (re.* (re.range "0" "9"))
                  (str.to.re "m"))))
(assert (= |4 Fill 6| (str.++ "\x1B[" |4 Fill 2| "m")))
(assert (let ((a!1 (>= (+ (str.len |0 Fill 4|) (* (- 1) (str.len Constructed_Argument)))
               0))
      (a!3 (ite (>= (+ (str.len |0 Fill 4|)
                       (str.len |0 Fill 6|)
                       (str.len Constructed_Argument))
                    0)
                (+ (str.len |0 Fill 4|)
                   (str.len |0 Fill 6|)
                   (str.len Constructed_Argument))
                0)))
(let ((a!2 (ite (<= (str.len Constructed_Argument) 0)
                ""
                (str.substr Constructed_Argument
                            0
                            (ite a!1
                                 (str.len Constructed_Argument)
                                 (str.len |0 Fill 4|)))))
      (a!4 (ite (>= (+ (str.len |0 Fill 4|) (str.len |0 Fill 6|)) 0)
                (+ (str.len |0 Fill 4|) (str.len |0 Fill 6|))
                a!3)))
(let ((a!5 (ite (<= (+ (str.len Constructed_Argument) (* (- 1) a!4)) 0)
                ""
                (str.substr Constructed_Argument
                            a!4
                            (+ (str.len Constructed_Argument) (* (- 1) a!4))))))
(let ((a!6 (ite (>= (+ (str.len |1 Fill 4|)
                       (str.len |1 Fill 6|)
                       (str.len a!2)
                       (str.len a!5))
                    0)
                (+ (str.len |1 Fill 4|)
                   (str.len |1 Fill 6|)
                   (str.len a!2)
                   (str.len a!5))
                0))
      (a!10 (>= (+ (str.len |1 Fill 4|)
                   (* (- 1) (str.len a!2))
                   (* (- 1) (str.len a!5)))
                0)))
(let ((a!7 (ite (>= (+ (str.len |1 Fill 4|) (str.len |1 Fill 6|)) 0)
                (+ (str.len |1 Fill 4|) (str.len |1 Fill 6|))
                a!6))
      (a!11 (str.substr (str.++ |1 Fill 4| "\x1B[" |1 Fill 2| "m" |1 Fill 5|)
                        0
                        (ite a!10
                             (+ (str.len a!2) (str.len a!5))
                             (str.len |1 Fill 4|)))))
(let ((a!8 (<= (+ (str.len a!2) (str.len a!5) (* (- 1) a!7)) 0))
      (a!9 (str.substr (str.++ |1 Fill 4| "\x1B[" |1 Fill 2| "m" |1 Fill 5|)
                       a!7
                       (+ (str.len a!2) (str.len a!5) (* (- 1) a!7))))
      (a!12 (ite (<= (+ (str.len a!2) (str.len a!5)) 0) 0 (str.len a!11)))
      (a!14 (ite (<= (+ (str.len a!2) (str.len a!5)) 0) "" a!11)))
(let ((a!13 (<= (+ (ite a!8 0 (str.len a!9)) a!12) 0))
      (a!15 (+ (str.len |2 Fill 4|)
               (* (- 1) (ite a!8 0 (str.len a!9)))
               (* (- 1) a!12)))
      (a!18 (+ (str.len |2 Fill 4|)
               (str.len |2 Fill 6|)
               (ite a!8 0 (str.len a!9))
               a!12)))
(let ((a!16 (ite (>= a!15 0)
                 (+ (ite a!8 0 (str.len a!9)) a!12)
                 (str.len |2 Fill 4|)))
      (a!19 (ite (>= (+ (str.len |2 Fill 4|) (str.len |2 Fill 6|)) 0)
                 (+ (str.len |2 Fill 4|) (str.len |2 Fill 6|))
                 (ite (>= a!18 0) a!18 0))))
(let ((a!17 (str.len (str.substr (str.++ a!14 (ite a!8 "" a!9)) 0 a!16)))
      (a!20 (+ (ite a!8 0 (str.len a!9)) a!12 (* (- 1) a!19)))
      (a!23 (ite a!13 "" (str.substr (str.++ a!14 (ite a!8 "" a!9)) 0 a!16))))
(let ((a!21 (str.len (str.substr (str.++ a!14 (ite a!8 "" a!9)) a!19 a!20)))
      (a!24 (ite (<= a!20 0)
                 ""
                 (str.substr (str.++ a!14 (ite a!8 "" a!9)) a!19 a!20))))
(let ((a!22 (+ (ite a!13 0 a!17) (ite (<= a!20 0) 0 a!21)))
      (a!25 (+ (str.len |3 Fill 4|)
               (* (- 1) (ite a!13 0 a!17))
               (* (- 1) (ite (<= a!20 0) 0 a!21))))
      (a!27 (+ (str.len |3 Fill 4|)
               (str.len |3 Fill 6|)
               (ite a!13 0 a!17)
               (ite (<= a!20 0) 0 a!21))))
(let ((a!26 (ite (<= a!22 0)
                 ""
                 (str.substr (str.++ a!23 a!24)
                             0
                             (ite (>= a!25 0) a!22 (str.len |3 Fill 4|)))))
      (a!28 (ite (>= (+ (str.len |3 Fill 4|) (str.len |3 Fill 6|)) 0)
                 (+ (str.len |3 Fill 4|) (str.len |3 Fill 6|))
                 (ite (>= a!27 0) a!27 0))))
(let ((a!29 (+ (ite a!13 0 a!17) (ite (<= a!20 0) 0 a!21) (* (- 1) a!28))))
(let ((a!30 (str.++ a!26
                    (ite (<= a!29 0)
                         ""
                         (str.substr (str.++ a!23 a!24) a!28 a!29)))))
(let ((a!31 (str.in.re a!30
                       (re.++ (re.* (re.range "\x00" "\xff"))
                              (str.to.re "\x1B[")
                              (re.range "0" "9")
                              (re.* (re.range "0" "9"))
                              (str.to.re "m")
                              (re.* (re.range "\x00" "\xff"))))))
  (or (not a!31) (= a!30 (str.++ |4 Fill 4| |4 Fill 6| |4 Fill 5|)))))))))))))))))))
(assert (let ((a!1 (>= (+ (str.len |0 Fill 4|) (* (- 1) (str.len Constructed_Argument)))
               0))
      (a!3 (ite (>= (+ (str.len |0 Fill 4|)
                       (str.len |0 Fill 6|)
                       (str.len Constructed_Argument))
                    0)
                (+ (str.len |0 Fill 4|)
                   (str.len |0 Fill 6|)
                   (str.len Constructed_Argument))
                0)))
(let ((a!2 (ite (<= (str.len Constructed_Argument) 0)
                ""
                (str.substr Constructed_Argument
                            0
                            (ite a!1
                                 (str.len Constructed_Argument)
                                 (str.len |0 Fill 4|)))))
      (a!4 (ite (>= (+ (str.len |0 Fill 4|) (str.len |0 Fill 6|)) 0)
                (+ (str.len |0 Fill 4|) (str.len |0 Fill 6|))
                a!3)))
(let ((a!5 (ite (<= (+ (str.len Constructed_Argument) (* (- 1) a!4)) 0)
                ""
                (str.substr Constructed_Argument
                            a!4
                            (+ (str.len Constructed_Argument) (* (- 1) a!4))))))
(let ((a!6 (>= (+ (str.len |1 Fill 4|)
                  (* (- 1) (str.len a!2))
                  (* (- 1) (str.len a!5)))
               0))
      (a!9 (ite (>= (+ (str.len |1 Fill 4|)
                       (str.len |1 Fill 6|)
                       (str.len a!2)
                       (str.len a!5))
                    0)
                (+ (str.len |1 Fill 4|)
                   (str.len |1 Fill 6|)
                   (str.len a!2)
                   (str.len a!5))
                0)))
(let ((a!7 (str.substr (str.++ |1 Fill 4| "\x1B[" |1 Fill 2| "m" |1 Fill 5|)
                       0
                       (ite a!6
                            (+ (str.len a!2) (str.len a!5))
                            (str.len |1 Fill 4|))))
      (a!10 (ite (>= (+ (str.len |1 Fill 4|) (str.len |1 Fill 6|)) 0)
                 (+ (str.len |1 Fill 4|) (str.len |1 Fill 6|))
                 a!9)))
(let ((a!8 (ite (<= (+ (str.len a!2) (str.len a!5)) 0) "" a!7))
      (a!11 (ite (<= (+ (str.len a!2) (str.len a!5) (* (- 1) a!10)) 0)
                 ""
                 (str.substr (str.++ |1 Fill 4|
                                     "\x1B["
                                     |1 Fill 2|
                                     "m"
                                     |1 Fill 5|)
                             a!10
                             (+ (str.len a!2) (str.len a!5) (* (- 1) a!10))))))
(let ((a!12 (>= (+ (str.len |2 Fill 4|)
                   (* (- 1) (str.len a!8))
                   (* (- 1) (str.len a!11)))
                0))
      (a!15 (ite (>= (+ (str.len |2 Fill 4|)
                        (str.len |2 Fill 6|)
                        (str.len a!8)
                        (str.len a!11))
                     0)
                 (+ (str.len |2 Fill 4|)
                    (str.len |2 Fill 6|)
                    (str.len a!8)
                    (str.len a!11))
                 0)))
(let ((a!13 (str.substr (str.++ a!8 a!11)
                        0
                        (ite a!12
                             (+ (str.len a!8) (str.len a!11))
                             (str.len |2 Fill 4|))))
      (a!16 (ite (>= (+ (str.len |2 Fill 4|) (str.len |2 Fill 6|)) 0)
                 (+ (str.len |2 Fill 4|) (str.len |2 Fill 6|))
                 a!15)))
(let ((a!14 (ite (<= (+ (str.len a!8) (str.len a!11)) 0) 0 (str.len a!13)))
      (a!17 (<= (+ (str.len a!8) (str.len a!11) (* (- 1) a!16)) 0))
      (a!18 (str.substr (str.++ a!8 a!11)
                        a!16
                        (+ (str.len a!8) (str.len a!11) (* (- 1) a!16))))
      (a!20 (ite (<= (+ (str.len a!8) (str.len a!11)) 0) "" a!13)))
(let ((a!19 (<= (+ a!14 (ite a!17 0 (str.len a!18))) 0))
      (a!21 (+ (str.len |3 Fill 4|)
               (* (- 1) a!14)
               (* (- 1) (ite a!17 0 (str.len a!18)))))
      (a!24 (+ (str.len |3 Fill 4|)
               (str.len |3 Fill 6|)
               a!14
               (ite a!17 0 (str.len a!18)))))
(let ((a!22 (ite (>= a!21 0)
                 (+ a!14 (ite a!17 0 (str.len a!18)))
                 (str.len |3 Fill 4|)))
      (a!25 (ite (>= (+ (str.len |3 Fill 4|) (str.len |3 Fill 6|)) 0)
                 (+ (str.len |3 Fill 4|) (str.len |3 Fill 6|))
                 (ite (>= a!24 0) a!24 0))))
(let ((a!23 (ite a!19 "" (str.substr (str.++ a!20 (ite a!17 "" a!18)) 0 a!22)))
      (a!26 (+ a!14 (ite a!17 0 (str.len a!18)) (* (- 1) a!25))))
(let ((a!27 (ite (<= a!26 0)
                 ""
                 (str.substr (str.++ a!20 (ite a!17 "" a!18)) a!25 a!26))))
(let ((a!28 (str.in.re (str.++ a!23 a!27)
                       (re.++ (re.* (re.range "\x00" "\xff"))
                              (str.to.re "\x1B[")
                              (re.range "0" "9")
                              (re.* (re.range "0" "9"))
                              (str.to.re "m")
                              (re.* (re.range "\x00" "\xff"))))))
  (= a!28 |IsMatch_/\\x1B\\[\\d+m/_4|))))))))))))))))
(assert (let ((a!1 (>= (+ (str.len |0 Fill 4|) (* (- 1) (str.len Constructed_Argument)))
               0))
      (a!3 (ite (>= (+ (str.len |0 Fill 4|)
                       (str.len |0 Fill 6|)
                       (str.len Constructed_Argument))
                    0)
                (+ (str.len |0 Fill 4|)
                   (str.len |0 Fill 6|)
                   (str.len Constructed_Argument))
                0)))
(let ((a!2 (ite (<= (str.len Constructed_Argument) 0)
                ""
                (str.substr Constructed_Argument
                            0
                            (ite a!1
                                 (str.len Constructed_Argument)
                                 (str.len |0 Fill 4|)))))
      (a!4 (ite (>= (+ (str.len |0 Fill 4|) (str.len |0 Fill 6|)) 0)
                (+ (str.len |0 Fill 4|) (str.len |0 Fill 6|))
                a!3)))
(let ((a!5 (ite (<= (+ (str.len Constructed_Argument) (* (- 1) a!4)) 0)
                ""
                (str.substr Constructed_Argument
                            a!4
                            (+ (str.len Constructed_Argument) (* (- 1) a!4))))))
(let ((a!6 (>= (+ (str.len |1 Fill 4|)
                  (* (- 1) (str.len a!2))
                  (* (- 1) (str.len a!5)))
               0))
      (a!9 (ite (>= (+ (str.len |1 Fill 4|)
                       (str.len |1 Fill 6|)
                       (str.len a!2)
                       (str.len a!5))
                    0)
                (+ (str.len |1 Fill 4|)
                   (str.len |1 Fill 6|)
                   (str.len a!2)
                   (str.len a!5))
                0)))
(let ((a!7 (str.substr (str.++ a!2 a!5)
                       0
                       (ite a!6
                            (+ (str.len a!2) (str.len a!5))
                            (str.len |1 Fill 4|))))
      (a!10 (ite (>= (+ (str.len |1 Fill 4|) (str.len |1 Fill 6|)) 0)
                 (+ (str.len |1 Fill 4|) (str.len |1 Fill 6|))
                 a!9)))
(let ((a!8 (ite (<= (+ (str.len a!2) (str.len a!5)) 0) "" a!7))
      (a!11 (ite (<= (+ (str.len a!2) (str.len a!5) (* (- 1) a!10)) 0)
                 ""
                 (str.substr (str.++ a!2 a!5)
                             a!10
                             (+ (str.len a!2) (str.len a!5) (* (- 1) a!10))))))
(let ((a!12 (>= (+ (str.len |2 Fill 4|)
                   (* (- 1) (str.len a!8))
                   (* (- 1) (str.len a!11)))
                0))
      (a!15 (ite (>= (+ (str.len |2 Fill 4|)
                        (str.len |2 Fill 6|)
                        (str.len a!8)
                        (str.len a!11))
                     0)
                 (+ (str.len |2 Fill 4|)
                    (str.len |2 Fill 6|)
                    (str.len a!8)
                    (str.len a!11))
                 0)))
(let ((a!13 (str.substr (str.++ a!8 a!11)
                        0
                        (ite a!12
                             (+ (str.len a!8) (str.len a!11))
                             (str.len |2 Fill 4|))))
      (a!16 (ite (>= (+ (str.len |2 Fill 4|) (str.len |2 Fill 6|)) 0)
                 (+ (str.len |2 Fill 4|) (str.len |2 Fill 6|))
                 a!15)))
(let ((a!14 (ite (<= (+ (str.len a!8) (str.len a!11)) 0) "" a!13))
      (a!17 (ite (<= (+ (str.len a!8) (str.len a!11) (* (- 1) a!16)) 0)
                 ""
                 (str.substr (str.++ a!8 a!11)
                             a!16
                             (+ (str.len a!8) (str.len a!11) (* (- 1) a!16))))))
(let ((a!18 (>= (+ (str.len |3 Fill 4|)
                   (* (- 1) (str.len a!14))
                   (* (- 1) (str.len a!17)))
                0))
      (a!21 (ite (>= (+ (str.len |3 Fill 4|)
                        (str.len |3 Fill 6|)
                        (str.len a!14)
                        (str.len a!17))
                     0)
                 (+ (str.len |3 Fill 4|)
                    (str.len |3 Fill 6|)
                    (str.len a!14)
                    (str.len a!17))
                 0)))
(let ((a!19 (str.substr (str.++ a!14 a!17)
                        0
                        (ite a!18
                             (+ (str.len a!14) (str.len a!17))
                             (str.len |3 Fill 4|))))
      (a!22 (ite (>= (+ (str.len |3 Fill 4|) (str.len |3 Fill 6|)) 0)
                 (+ (str.len |3 Fill 4|) (str.len |3 Fill 6|))
                 a!21)))
(let ((a!20 (ite (<= (+ (str.len a!14) (str.len a!17)) 0) "" a!19))
      (a!23 (ite (<= (+ (str.len a!14) (str.len a!17) (* (- 1) a!22)) 0)
                 ""
                 (str.substr (str.++ a!14 a!17)
                             a!22
                             (+ (str.len a!14) (str.len a!17) (* (- 1) a!22))))))
  (str.in.re (str.++ a!20 a!23)
             (re.++ (re.* (re.range "\x00" "\xff"))
                    (str.to.re "\x1B[")
                    (re.range "0" "9")
                    (re.* (re.range "0" "9"))
                    (str.to.re "m")
                    (re.* (re.range "\x00" "\xff")))))))))))))))))

(check-sat)