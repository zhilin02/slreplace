...
\begin{itemize}
\item At first, we compute $(\cT_{\sf nameReg})^{-1}(.*,.*)$,  i.e. the pre-image of the language $.*,.*$ under $\cT_{\sf nameReg}$,  remove the assignment $\sf  ret  := \sf  \cT_{nameReg}(authorList)$, and add the assertion $\ASSERT{\sf authorList \in (\cT_{\sf nameREg})^{-1}(.*,.*)}$, resulting into the following program,
\begin{eqnarray}\label{eqn:exmp}
& & \ASSERT{\sf authorList \in autListReg};\nonumber \\
%& & \sf ret  := \sf  \cT_{nameReg}(authorList);\nonumber \\
& &  \ASSERT{\sf ret \in .*,.*}; \nonumber \\
