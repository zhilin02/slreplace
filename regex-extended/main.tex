...
For a \regexp[\sf CG] $e$, we define a pNFA $A_e$ by a modified version of
Glushkov construction\cite{Gluskov61}. The original construction on regular expression
produces an NFA with no $\varepsilon$-transition, which we denote by $G (e)$.
Each occurrence of character is first labeled, and then for each subexpression
$v$ of $e$, the sets $\tmop{Start} (v)$, $\tmop{End} (v)$ and $\tmop{Pair} (v)$
are computed, which stand for starting, end and consecutive character
occurrences in v. The state of $G (e)$ comprises $q_0$, a special starting
state, and all the labeled character occurrences. The transitions of $G (e)$
depend on the three special sets, the details of which are elaborate and
omitted here. Refer to \cite{Gluskov61} for a formal definition.


The pNFA $A_e$ is obtained by adding priority to $G (e)$, and we define the
priority recursively on the parse tree of e.

Suppose $G' (e)=G (e) \backslash \{ q_0 \}$. For the
subexpressions $e_1$ and $e_2$ defined below, let $\tmop{Start} (e_1) = a_1
\ldots a_n$, $\tmop{End} (e_1) = b_1 \ldots b_m$, $\tmop{Start} (e_2) = c_1 \ldots
c_p$, and $\tmop{End} (e_2) = d_1 \ldots d_q$, where in $G (e_1)$ and $G (e_2)$, the
transition from $q_0$ to $a_1$ and $c_1$ is of highest priority, and $a_n$ and
$c_p$ the lowest.

We illustrate the construction in Figure. \ref{fig:pglushkov}, where a transition with lower
priority is dotted and thiner. The cases when $e = \varepsilon$, $e = a$ and $e
= (e_1)$ are routine and thus omitted. Note when $e = e_1 \cdot e_2$, only one
of totally four circumstances is shown, i.e. $\varepsilon \in L (e_1) \wedge
\varepsilon \nin L (e_2)$. The construction in other three circumstances is
similar.
