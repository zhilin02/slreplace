...
A  run of $\pnfa$ is the sequence $q_0 \sigma_1 q_1 \ldots \sigma_m q_m$ such that 
\begin{itemize}
%\item $q_m \in F$,
\item for any $i \in [m]$, either $\sigma_i \in \Sigma$ and $q_i \in \delta (q_{i - 1}, \sigma_i)$, or $\sigma_i = \varepsilon$ and $q_i \in \tau(q_{i-1)}$ %\pi_1(\tau(q_{i-1}))\cup \pi_2(\tau(q_{i-1}))$,
