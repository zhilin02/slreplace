...
\tl{another way is to first define B as a PST, could make the construction a bit modular?}
 \zhilin{this is a bug in this proof, which should be cleaned.}
...
Moreover, for $S \subseteq Q_T$, we define $\tau^{(ip)}_T(S) = \bigcup \limits_{q \in S} \tau^{(ip)}_T(\{q\})$. We also use $(\tau^{(ip)}_T)^\ast$ to denote $\bigcup \limits_{n \in \Nat} (\tau^{(ip)}_T)^{n}(S)$, where $(\tau^{(ip)}_T)^{0}(S) = S$, and for $n \in \Nat$, $(\tau^{(ip)}_T)^{n+1}(S) = \tau^{(ip)}_T((\tau^{(ip)}_T)^{n}(S))$. 
%
...
\begin{itemize}
\item $Q_B = \{q'_0\} \cup Q_T \times (\Sigma \cup \{\bot\}) \times (\cP(Q_A \times Q_A ))^{X} \times \cP(Q_T)$, (Intuitively, the letter $\sigma$ in $(q, \sigma, \rho, S) \in Q_B$ means the next letter to be read at $q$, with $\bot$ represents the end of the input.)
...
%
\item the tuples $(((q,\sigma), \rho, S), \sigma, ((q_i,\sigma'), \rho', S'))$ such that  
%there exists $s \in \left((X \cup \Sigma\right)^*)^X)$ satisfying
\begin{itemize}
\item $\sigma \in \Sigma$, $\sigma' \in \Sigma \cup \{\bot\}$, 
%
...
\item $\rho' = s(\rho)$,
%
\item let $\tau_T(q) = (P_1, P_2)$ and $(\tau^{(ip)}_T)^\ast(S \cup P_1) = S''$, then $S' =  \delta^{(ip)}_T(S'', \sigma)$ $\cup$ $\{ q_1$, $\ldots$, $q_{i - 1} \}$; 
%
...
\end{itemize}
%
\item the tuples $(((q, \sigma), \rho, S), \varepsilon, ((q_i,\sigma), \rho', S'))$ such that 
%there exists $s \in \left((X \cup \Sigma\right)^*)^X$ satisfying
\begin{itemize}
\item $\sigma \in \Sigma  \cup \{\bot\}$
...
\item $\rho' = s(\rho)$,
%
\item if $\sigma = \bot$, then $S' =\tau^{(ip)}_T(S)$ $\cup$  $\{q'_1, \ldots, q'_n, q_1$, $\ldots$, $q_{i - 1} \}$, otherwise, $S' =\tau^{(ip)}_T(S) \cup  \delta_T(S \cup \{q\}, \sigma) \cup \{q'_1, \ldots, q'_n, q_1, \ldots, q_{i - 1} \}$.
%
%$\rho'(x) = \theta_\ell$ such that $\theta_0 = \{(p,p) \mid p \in Q_A\}$, and for each $i \in [\ell]$, if $b_i \in \Sigma$, then $\theta_i = \{(p, p') \mid (p, p'') \in \theta_{i-1}, (p'', b_i, p') \in \delta_A \mbox{ for some } p''\}$, otherwise, $\theta_i = \theta_{i-1} \cdot \rho(x)$. 
\end{itemize}
\end{itemize}
