...
We are ready to present the construction of $\cB =  (Q_B$, $\Sigma$, $\delta_B$, $q_{0, B}, F_B)$: $Q_B = \{q'_0\} \cup Q_T \times (\Sigma \cup \{\bot\}) \times (\cP(Q_A \times Q_A ))^{X} \times \cP(Q_T)  $, $q_{0, B} = q'_0$, 
$\delta_{B}$ comprises 
\begin{itemize}
\item $(q'_0, )$, $(q_{0, T}, \rho_{\varepsilon}, \emptyset)$ where $\rho_{\varepsilon} (x) = \{(q, q) \mid q \in Q\}$ for each $x \in X$, 
%
...
%
\item $s = E(q, \sigma, q_i)$, \tl{here $s$ must be this, so no need to quantify $s$?}
%
\item let $\tau^{(ip)}_T(q) = (S_1, S_2)$ and $(\tau^{(ip)}_T)^\ast(S \cup S_1) = S''$, then $S' =  \delta^{(ip)}_T(S'', \sigma)$ $\cup$ $\{ q_1$, $\ldots$, $q_{i - 1} \}$, 
\item $\rho' = s(\rho)$;
%
