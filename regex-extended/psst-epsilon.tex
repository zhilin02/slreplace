...
\begin{remark}
In each run of $\pnfa$ on a given string $w$, it is required that each state occurs at most twice in a sequence of $\varepsilon$-transitions. This requirement is necessary for bounding the length of the runs on $w$, thus guaranteeing the existence of a run of highest priority on $w$. The choice of ``at most twice'', instead of ``at most once'', is for technical reasons. 
\end{remark}
...
Let $\psst = (Q_T, \Sigma$, $X, \delta_T, \tau_T, E_T,  q_{0, T}, F_T)$ be \PSST{}  and $\Aut
  = (Q_A, \Sigma, \delta_A, q_{0, A}, F_A)$ be an \FA{}.
Without loss of generality, we assume that $\Aut$ contains no $\varepsilon$-transitions. 

To illustrate the intuition of the FA construction, let us start with the following natural idea of constructing a PFA $\cB$ for the pre-image: $\cB$ simulates the run of $\psst$ on $w$, and, for each $x \in X$, records an $\Aut$-abstraction of the string stored in $x$, that is, the set of state pairs $(p, q) \in Q_A \times Q_A$ such that starting from $p$, $\Aut$ can reach $q$ after reading the string stored in $x$. Specifically, the states of $\cB$ are of the form $(q, \rho)$ with $q \in Q$ and $\rho \in (\cP(Q_A \times Q_A ))^{X}$, moreover, the priorities of $\cB$ inherit those of $\Aut$. 

Nevertheless, it turns out that this PFA construction is flawed: A string $w$ is in $\cR^{-1}_{\cT}(\Lang(\Aut))$ iff the (unique) accepting run of $\cT$ on $w$ produces an output $w'$ that is accepted by $\Aut$. In contrast, a string $w$ is accepted by the PFA $\cB$ iff there is a run of $\cT$ on $w$, not necessarily of the highest priority, producing an output $w'$ that is accepted by $\Aut$. The following example illustrates the flaw of the PFA construction. 
...
\begin{example}
Figure.\ref{fig-psst-preimage-exmp} gives a simple example of \PSST{} Pre-image computation. $\cT$ is a basic \PSST{} that reads \textit{a} from the input and appends it to string variable \textit{x}, which will be the output. At any point it could stop and keep idle by entering $q_1$. $\Aut$ is a trivial FA which accepts mere $aa$. The FA $\cB$ encodes the Pre-image $\cR^{-1}_{\cT}(\Lang(\Aut))$. The states of $\cB$ are labeled the triple mentioned in the algorithm above.

\begin{figure*}[ht]
\centering
%\rule{\linewidth}{0cm}
\includegraphics[scale=0.8]{psst-preimage.pdf}
\caption{\textit{a)} The PSST $\cT$. \textit{b)} The FA $\Aut$ \textit{c)} The FA $\cB$ which encodes $\cR^{-1}_{\cT}(\Lang(\Aut))$}
