...
%
\item For $q \in Q_T$,  if $\tau_T(q) = (P_1, P_2)$, then $\tau^{(ip)}_T(\{q\})=S$ such that $S$ is the set of states occurring in $P_1$ and $P_2$. 
\tl{a bit confused, why not write $S=P_1\cup P_2$?} Moreover, for $S \subseteq Q_T$, we define $\tau^{(ip)}_T(S) = \bigcup \limits_{q \in S} \tau^{(ip)}_T(\{q\})$. 
%
...
\begin{itemize}
\item the tuples $((q, \rho, S), \sigma, (q_i, \rho', S'))$ such that $\sigma \in \Sigma$ and there exists $s \in X\rightarrow \left((X \cup \Sigma\right)^*)$ satisfying
\begin{itemize}
