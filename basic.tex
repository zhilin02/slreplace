...
\noindent{\bf Step I.} We utilise the dependency graph $C$ and compute a collection of atomic regular constraints $\cE_x$ for each variable $x$, in a top-down manner as follows. 
...
\noindent {\bf Step II.} For each source variable $y$, guess an accepting run of the product automaton of all the NFA in $\cE_y$. 
