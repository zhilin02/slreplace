...
\noindent{\bf Step I.} We utilise the dependency graph $C$ and compute nondeterministically a collection of atomic regular constraints $\cE(x)$ for each variable $x$, in a top-down manner as follows. 

During the computation, for each variable $x$, the collection of atomic regular constraints $\cE(x)$ is represented in a succinct way: That is, as a set of pairs $(\cT, \cP)$, where $\cT=(Q, \delta)$ is a transition graph and $\cP \subseteq Q \times Q$. The intention of $(\cT, \cP)$ is to represent succinctly the collection of the atomic regular constraints $(Q, \delta, q, \{q'\})$, where $q$ is the initial state and $\{q'\}$ is the set of final states, for $(q,q') \in \cP$.

Initially, let $G_0:= G_C$. In addition, for each variable $x$, compute $\cE_0(x)$ as follows: Initially, let $\cE_0(x) := \emptyset$.
...
\begin{enumerate}
\item Select a node $x$ of $G_i$ such that $x$ has no predecessors and has two successors in $G_i$. Suppose $(x, (\rpleft, a), y)$ and $(x, (\rpright,a), z)$ are the two edges out of $x$ in $G_i$ and $\cE_i(x)=\{(\cT_1,\cP_1), \cdots, (\cT_k, \cP_k)\}$. For each $j \in [k]$, let $\cT_j = (Q_j, \delta_j)$. Then $\cE_{i+1}(z)$ and  $\cE_{i+1}(y)$ and $G_{i+1}$ are computed as follows:  
\begin{enumerate}
...
In addition, for each node $x'$ distinct from $y, z$, let $\cE_{i+1}(x') := \cE_i(x')$.
%
...
\begin{proposition}
The cardinalities of $\cE(x)$ for variables $x$ in $G_C$ are at most exponential w.r.t. the size of $C$, and polynomial if $G_C$ is a tree.
\end{proposition}
...
Let $K$ denote the maximum cardinality of $\cE_0(x)$ for nodes $x$ in $G_C$. 
...
For each $i$ and each node $x$ in $G_C$,  let $\#^{\sf anc}_i(x)$ denote the number of ancestors of $x$ in $G_i$.
...
\noindent {\bf Claim}. For each $i$ and each node $x$ in $G_C$, 
$|\cE_i(x)| \le 3^i K$. In addition, if $G_C$ is a tree, then $|\cE_i(x)| \le (\#^{\sf anc}_0(x)-\#^{\sf anc}_i(x)+1) K$.
...
\noindent {\it Induction base}: $i=0$. Evidently $|\cE_0(x)| \le K = 3^i K = (\#^{\sf anc}_0(x)-\#^{\sf anc}_0(x)+1) K$.
...
Let $x$ be the node without predecessors and with successors in $G_{i-1}$ that is used to construct $G_{i}$. In addition, let $(x, (\rpleft, a), y)$ and $(x, (\rpright, a), z)$ be the two edges out of $x$ in $G_{i-1}$.
...
For the special case that $G_C$ is a tree, we know that $y \neq z$. Then $\cE_{i-1}(z) = \cE_0(z)$ and $\cE_{i-1}(y) = \cE_0(y)$. Therefore, $|\cE_{i}(z)| \le |\cE_{i-1}(z)| + |\cE_{i-1}(x)| \le K + |\cE_{i-1}(x)|$. By the induction hypothesis, $|\cE_{i-1}(x)| \le (\#^{\sf anc}_{0}(x) - \#^{\sf anc}_{i-1}(x)+1)K$. We deduce that $|\cE_{i}(z)| \le K+ (\#^{\sf anc}_{0}(x) - \#^{\sf anc}_{i-1}(x)+1)K$. Moreover, since $\#^{\sf anc}_{0}(z) = \#^{\sf anc}_{0}(x)+1$ and $\#^{\sf anc}_{i-1}(x)=\#^{\sf anc}_{i}(x)= \#^{\sf anc}_{i}(z)=0$, we have $|\cE_{i}(z)| \le K+ (\#^{\sf anc}_{0}(z)-1 - \#^{\sf anc}_{i}(z)+1)K = (\#^{\sf anc}_{0}(z) - \#^{\sf anc}_{i}(z)+1)K$.
Similarly, $|\cE_{i}(y)| \le |\cE_{i-1}(y)| + |\cE_{i-1}(x)| = |\cE_{0}(y)| + |\cE_{i-1}(x)| \le K+|\cE_{i-1}(x)| \le K+ (\#^{\sf anc}_{0}(x) - \#^{\sf anc}_{i-1}(x)+1)K \le K+ (\#^{\sf anc}_{0}(y)-1 - \#^{\sf anc}_{i}(y)+1)K = (\#^{\sf anc}_{0}(y) - \#^{\sf anc}_{i}(y)+1)K$.


%From the construction, we know that $\#^{\sf rmv}_{i-1}(x)  = \#^{\sf rmv}_{i}(x)$, $ \#^{\sf rmv}_{i}(y) = \#^{\sf rmv}_{i-1}(y)-1$, and  $ \#^{\sf cor}_{i}(z) = \#^{\sf cor}_{i-1}(z)-1$. 

%Otherwise, $|\cG(\cE_{i+1}(z))| \le |\cG(\cE_{i}(z))| + |\cE_{i}(x)| + |\cE_{i}(x)| \le K_i + K_i + K_i = 3K_i$. Therefore, we deduce that $K_{i+1} \le 3K_i$.
\end{proof}
