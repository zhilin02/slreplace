...
\begin{proposition}
The cardinalities of $\cE(x)$ for variables $x$ in $G_C$ are at most exponential w.r.t. $\arbidx(G_C)$, the arborescence index of $G_C$.
\end{proposition}
...
For each $i$ and each vertex $x$ in $G_C$, let  $\#^{\sf anc}_i(x)$ denote the number of ancestors of $x$ in $G_i$, in addition, let ${\sf Idx}^{\sf arb}_i(x)$ denote the maximum number of join vertices in a path starting from $x$ in $G_i$.
...
$|\cE_i(x)| \le 3^{{\sf Idx}^{\sf arb}_0(x)-{\sf Idx}^{\sf arb}_i(x)} K$. 
...
\noindent {\it Induction base}: $i=0$. Evidently $|\cE_0(x)| \le K = 3^{{\sf Idx}^{\sf arb}_0(x)-{\sf Idx}^{\sf arb}_0(x)} K$.
